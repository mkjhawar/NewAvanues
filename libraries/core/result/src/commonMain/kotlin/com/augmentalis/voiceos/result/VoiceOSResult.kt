...
sealed class VoiceOSResult<out T> {
    ...
    fun getOrThrow(): T = when (this) {
        is Success -> data
        is Failure -> throw VoiceOSException(error)
...
/**
 * Exception wrapper for VoiceOSError
 *
 * Used when exceptions are required (e.g., getOrThrow()).
 */
class VoiceOSException(val error: VoiceOSError) : Exception(error.message, error.cause) {
    /**
     * Get error code
     *
    ...
    /**
     * Get full error message with cause chain
     *
     * @return Full error message
     */
    fun getFullMessage(): String = error.getFullMessage()
}
