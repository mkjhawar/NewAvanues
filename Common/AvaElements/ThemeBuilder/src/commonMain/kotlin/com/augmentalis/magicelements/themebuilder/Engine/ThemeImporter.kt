package com.augmentalis.avaelements.themebuilder.Engine
...
class ThemeImporter {
    ...
    private fun parseShapes(dsl: String): Shapes {
        ...
        return Shapes(
            extraSmall = extractFloat(shapesSection, "extraSmall") ?: 4f,
            small = extractFloat(shapesSection, "small") ?: 8f,
            medium = extractFloat(shapesSection, "medium") ?: 12f,
            large = extractFloat(shapesSection, "large") ?: 16f,
            extraLarge = extractFloat(shapesSection, "extraLarge") ?: 28f
        )
    }
     ...
     */
    private fun parseSpacing(dsl: String): Spacing {
     ...
     */
    private fun parseElevation(dsl: String): Elevation {
        val elevationSection = extractSection(dsl, "elevation")

        return Elevation(
            level0 = extractFloat(elevationSection, "level0") ?: 0f,
            level1 = extractFloat(elevationSection, "level1") ?: 1f,
            level2 = extractFloat(elevationSection, "level2") ?: 3f,
            level3 = extractFloat(elevationSection, "level3") ?: 6f,
            level4 = extractFloat(elevationSection, "level4") ?: 8f,
            level5 = extractFloat(elevationSection, "level5") ?: 12f
        )
    }
     ...
     * Extract font weight
     */
    private fun extractFontWeight(section: String): FontWeight {
        val regex = """weight\s*=\s*FontWeight\.(\w+)""".toRegex()
        val weightStr = regex.find(section)?.groupValues?.get(1) ?: "Normal"
        return try {
            FontWeight.valueOf(weightStr)
        } catch (e: Exception) {
            FontWeight.Normal
        }
    }
    ...
    private fun calculateRelativeLuminance(color: Color): Float {
        ...
        val rSrgb = if (r <= 0.03928f) r / 12.92f else kotlin.math.pow((r + 0.055f) / 1.055f, 2.4f).toFloat()
        val gSrgb = if (g <= 0.03928f) g / 12.92f else kotlin.math.pow((g + 0.055f) / 1.055f, 2.4f).toFloat()
        val bSrgb = if (b <= 0.03928f) b / 12.92f else kotlin.math.pow((b + 0.055f) / 1.055f, 2.4f).toFloat()
