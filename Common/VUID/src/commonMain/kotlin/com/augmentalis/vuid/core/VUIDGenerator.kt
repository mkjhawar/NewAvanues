/**
 * VUIDGenerator.kt - Cross-Platform VUID Generation
 *
 ...
 * Created: 2025-12-30
 *
 * Responsibility: Generate unique VUIDs (Voice Unique Identifiers) across all platforms
 *
...
object VUIDGenerator {
    ...
    /**
     * Generate compact VUID for third-party apps (VoiceOS)
     *
    ...
    /**
     * Generate compact VUID for internal module entities
     *
    ...
    /**
     * Generate simple compact VUID (no version)
     *
    ...
    // AVA Types
    fun generateMessageVuid(): String = generateCompactSimple(Module.AVA, "message")
    fun generateConversationVuid(): String = generateCompactSimple(Module.AVA, "conversation")
    fun generateDocumentVuid(): String = generateCompactSimple(Module.AVA, "document")
    fun generateMemoryVuid(): String = generateCompactSimple(Module.AVA, "memory")
    fun generateLearningVuid(): String = generateCompactSimple(Module.AVA, "learning")
    fun generateIntentVuid(): String = generateCompactSimple(Module.AVA, "intent")
    fun generateChunkVuid(): String = generateCompactSimple(Module.AVA, "chunk")
    fun generateClusterVuid(): String = generateCompactSimple(Module.AVA, "cluster")
    fun generateBookmarkVuid(): String = generateCompactSimple(Module.AVA, "bookmark")
    fun generateAnnotationVuid(): String = generateCompactSimple(Module.AVA, "annotation")
    fun generateFilterPresetVuid(): String = generateCompactSimple(Module.AVA, "filterpreset")
    fun generateUtteranceVuid(): String = generateCompactSimple(Module.AVA, "utterance")
    fun generateDecisionVuid(): String = generateCompactSimple(Module.AVA, "decision")
    fun generateDialogVuid(): String = generateCompactSimple(Module.AVA, "dialog")

    // WebAvanue Types
    fun generateTabVuid(): String = generateCompactSimple(Module.WEBAVANUE, "tab")
    fun generateFavoriteVuid(): String = generateCompactSimple(Module.WEBAVANUE, "favorite")
    fun generateDownloadVuid(): String = generateCompactSimple(Module.WEBAVANUE, "download")
    fun generateHistoryVuid(): String = generateCompactSimple(Module.WEBAVANUE, "history")
    fun generateSessionVuid(): String = generateCompactSimple(Module.WEBAVANUE, "session")
    fun generateGroupVuid(): String = generateCompactSimple(Module.WEBAVANUE, "group")

    // Cockpit Types
    fun generateRequestVuid(): String = generateCompactSimple(Module.COCKPIT, "request")
    fun generateWindowVuid(): String = generateCompactSimple(Module.COCKPIT, "window")
    fun generateStreamVuid(): String = generateCompactSimple(Module.COCKPIT, "stream")
    fun generatePresetVuid(): String = generateCompactSimple(Module.COCKPIT, "preset")
    fun generateDeviceVuid(): String = generateCompactSimple(Module.COCKPIT, "device")
    fun generateSyncVuid(): String = generateCompactSimple(Module.COCKPIT, "sync")
    ...
    /**
     * Generate standard random VUID (legacy format)
    ...
    /**
     * Generate VUID with prefix (legacy format)
     */
    ...
    /**
     * Generate VUID for specific element type (legacy format)
     */
    ...
    /** Pattern for app VUID: {reversed.pkg}:{version}:{type}:{hash8} */
    private val COMPACT_APP_PATTERN = Regex(
        "^[a-z][a-z0-9]*(?:\\.[a-z][a-z0-9]*)+:[0-9]+(?:\\.[0-9]+)*:[a-z]{3}:[a-f0-9]{8}$",
        RegexOption.IGNORE_CASE
    )

    /** Pattern for module VUID with version: {module}:{version}:{type}:{hash8} */
    private val COMPACT_MODULE_PATTERN = Regex(
        "^[a-z]{3}:[0-9]+(?:\\.[0-9]+)*:[a-z]{3}:[a-f0-9]{8}$",
        RegexOption.IGNORE_CASE
    )

    /** Pattern for simple module VUID: {module}:{type}:{hash8} */
    ...
    /** Check if VUID is in compact app format */
    fun isCompactApp(vuid: String): Boolean = COMPACT_APP_PATTERN.matches(vuid)

    /** Check if VUID is in compact module format (with version) */
    fun isCompactModule(vuid: String): Boolean = COMPACT_MODULE_PATTERN.matches(vuid)

    /** Check if VUID is in compact simple format (no version) */
    fun isCompactSimple(vuid: String): Boolean = COMPACT_SIMPLE_PATTERN.matches(vuid)

    /** Check if VUID is in any compact format */
    fun isCompact(vuid: String): Boolean =
        isCompactApp(vuid) || isCompactModule(vuid) || isCompactSimple(vuid)
    ...
    /** Check if any valid VUID format (compact or legacy) */
    fun isValid(vuid: String): Boolean =
        isCompact(vuid) || isLegacyUuid(vuid) || isLegacyVoiceOS(vuid)
    ...
    /** Parsed VUID components */
    data class ParsedVuid(
        ...
        val format: VuidFormat      // Detected format
    ) {
        /** Get full type name */
        ...
        fun toVerbose(): String = when (format) {
            VuidFormat.COMPACT_APP -> "$packageName v$version $typeName [$hash]"
            VuidFormat.COMPACT_MODULE -> "$module v$version $typeName [$hash]"
            VuidFormat.COMPACT_SIMPLE -> "$module $typeName [$hash]"
            VuidFormat.LEGACY_VOICEOS -> "$packageName v$version $typeName [$hash]"
            VuidFormat.LEGACY_UUID -> "uuid [$hash]"
            VuidFormat.UNKNOWN -> "unknown [$hash]"
        }

        /** Reconstruct compact VUID string */
        fun toCompactVuid(): String = when (format) {
            VuidFormat.COMPACT_APP -> {
                val reversedPkg = reversePackage(packageName!!)
                "$reversedPkg:$version:$typeAbbrev:$hash"
            }
            VuidFormat.COMPACT_MODULE -> "$module:$version:$typeAbbrev:$hash"
            VuidFormat.COMPACT_SIMPLE -> "$module:$typeAbbrev:$hash"
     ...
     *
     * @param vuid VUID string in any supported format
     * @return Parsed components or null if invalid format
     */
    fun parse(vuid: String): ParsedVuid? = when {
        isCompactApp(vuid) -> parseCompactApp(vuid)
        isCompactModule(vuid) -> parseCompactModule(vuid)
        isCompactSimple(vuid) -> parseCompactSimple(vuid)
        isLegacyUuid(vuid) -> parseLegacyUuid(vuid)
        isLegacyVoiceOS(vuid) -> parseLegacyVoiceOS(vuid)
        else -> null
    }

    private fun parseCompactApp(vuid: String): ParsedVuid {
        ...
        return ParsedVuid(
            ...
            hash = hash,
            format = VuidFormat.COMPACT_APP
        )
    }

    private fun parseCompactModule(vuid: String): ParsedVuid {
        val parts = vuid.split(":")
        return ParsedVuid(
            packageName = null,
            module = parts[0],
            ...
            typeAbbrev = parts[2],
            hash = parts[3],
            format = VuidFormat.COMPACT_MODULE
        )
    }

    private fun parseCompactSimple(vuid: String): ParsedVuid {
        val parts = vuid.split(":")
        return ParsedVuid(
            packageName = null,
            module = parts[0],
            ...
            typeAbbrev = parts[1],
            hash = parts[2],
            format = VuidFormat.COMPACT_SIMPLE
        )
    }

    private fun parseLegacyUuid(vuid: String): ParsedVuid {
        return ParsedVuid(
            packageName = null,
            module = null,
            version = null,
            typeAbbrev = TypeAbbrev.ELEMENT,
            hash = vuid.replace("-", "").takeLast(8),
            format = VuidFormat.LEGACY_UUID
        )
    }

    private fun parseLegacyVoiceOS(vuid: String): ParsedVuid {
        val lastDotIdx = vuid.lastIndexOf('.')
        val dashIdx = vuid.indexOf('-', lastDotIdx)

        if (lastDotIdx < 0 || dashIdx < 0) {
            return ParsedVuid(null, null, null, TypeAbbrev.ELEMENT, vuid.takeLast(8), VuidFormat.LEGACY_VOICEOS)
        }

        val beforeType = vuid.substring(0, lastDotIdx)
        val typeName = vuid.substring(lastDotIdx + 1, dashIdx)
        val hash = vuid.substring(dashIdx + 1).take(8)
        ...
        return ParsedVuid(
            ...
            hash = hash,
            format = VuidFormat.LEGACY_VOICEOS
        )
    }
    ...
    fun migrateToCompact(legacyVuid: String, version: String? = null): String? {
        ...
        return when (parsed.format) {
            ...
            VuidFormat.LEGACY_UUID -> null // Can't determine package/type from UUID
            VuidFormat.COMPACT_APP, VuidFormat.COMPACT_MODULE, VuidFormat.COMPACT_SIMPLE -> legacyVuid
