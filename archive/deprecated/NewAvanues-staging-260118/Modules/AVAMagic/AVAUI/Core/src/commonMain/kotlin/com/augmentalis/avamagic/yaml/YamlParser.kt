 ...
 * YAML to DSL Parser
 *
 * Converts YAML UI definitions to MagicElements DSL component tree.
 *
 * Example YAML:
 * ```yaml
...
class YamlParser {
    /**
     * Parse YAML string to MagicUI component tree
     */
    fun parse(yaml: String): MagicUI {
    ...
    /**
     * Parse JSON representation to MagicUI
     */
    private fun parseJson(jsonString: String): MagicUI {
     ...
     */
    private fun parseComponent(scope: MagicUIScope, componentJson: JsonObject) {
    ...
    private fun parseColumn(scope: MagicUIScope, data: JsonObject) {
        scope.Column(
    ...
    private fun parseRow(scope: MagicUIScope, data: JsonObject) {
        scope.Row(
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
        ) {
    ...
    private fun parseContainer(scope: MagicUIScope, data: JsonObject) {
        scope.Container(
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
        ) {
    ...
    private fun parseScrollView(scope: MagicUIScope, data: JsonObject) {
        scope.ScrollView(
            id = data["id"]?.jsonPrimitive?.content,
    ...
    private fun parseCard(scope: MagicUIScope, data: JsonObject) {
        scope.Card(
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
        ) {
    ...
    private fun parseText(scope: MagicUIScope, data: JsonObject) {
        scope.Text(
    ...
    private fun parseButton(scope: MagicUIScope, data: JsonObject) {
        scope.Button(
            text = data["text"]?.jsonPrimitive?.content ?: "",
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
    ...
    private fun parseImage(scope: MagicUIScope, data: JsonObject) {
        scope.Image(
    ...
    private fun parseCheckbox(scope: MagicUIScope, data: JsonObject) {
        scope.Checkbox(
    ...
    private fun parseTextField(scope: MagicUIScope, data: JsonObject) {
        scope.TextField(
    ...
    private fun parseSwitch(scope: MagicUIScope, data: JsonObject) {
        scope.Switch(
            checked = data["checked"]?.jsonPrimitive?.boolean ?: false,
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
    ...
    private fun parseIcon(scope: MagicUIScope, data: JsonObject) {
        scope.Icon(
    ...
    private fun parseRadio(scope: MagicUIScope, data: JsonObject) {
    ...
    private fun parseSlider(scope: MagicUIScope, data: JsonObject) {
        scope.Slider(
    ...
    private fun parseDropdown(scope: MagicUIScope, data: JsonObject) {
        val optionsArray = data["options"]?.jsonArray ?: return
        val options = optionsArray.map { optionElement ->
            val optionObj = optionElement.jsonObject
            DropdownOption(
    ...
    private fun parseDatePicker(scope: MagicUIScope, data: JsonObject) {
        scope.DatePicker(
    ...
    private fun parseTimePicker(scope: MagicUIScope, data: JsonObject) {
        scope.TimePicker(
    ...
    private fun parseFileUpload(scope: MagicUIScope, data: JsonObject) {
        scope.FileUpload(
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
        ) {
    ...
    private fun parseSearchBar(scope: MagicUIScope, data: JsonObject) {
        scope.SearchBar(
            value = data["value"]?.jsonPrimitive?.content ?: "",
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
    ...
    private fun parseRating(scope: MagicUIScope, data: JsonObject) {
        scope.Rating(
            value = data["value"]?.jsonPrimitive?.float ?: 0f,
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
    ...
    private fun parseDialog(scope: MagicUIScope, data: JsonObject) {
        scope.Dialog(
    ...
    private fun parseToast(scope: MagicUIScope, data: JsonObject) {
        scope.Toast(
    ...
    private fun parseAlert(scope: MagicUIScope, data: JsonObject) {
        scope.Alert(
    ...
    private fun parseProgressBar(scope: MagicUIScope, data: JsonObject) {
        scope.ProgressBar(
            value = data["value"]?.jsonPrimitive?.float ?: 0f,
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
    ...
    private fun parseSpinner(scope: MagicUIScope, data: JsonObject) {
        scope.Spinner(
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
        ) {
    ...
    private fun parseBadge(scope: MagicUIScope, data: JsonObject) {
        scope.Badge(
    ...
    private fun parseTooltip(scope: MagicUIScope, data: JsonObject) {
        scope.Tooltip(
            content = data["content"]?.jsonPrimitive?.content ?: "",
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
    ...
    private fun parseAppBar(scope: MagicUIScope, data: JsonObject) {
        scope.AppBar(
            title = data["title"]?.jsonPrimitive?.content ?: "",
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
    ...
    private fun parseBottomNav(scope: MagicUIScope, data: JsonObject) {
    ...
    private fun parseTabs(scope: MagicUIScope, data: JsonObject) {
    ...
    private fun parseDrawer(scope: MagicUIScope, data: JsonObject) {
        scope.Drawer(
            isOpen = data["isOpen"]?.jsonPrimitive?.boolean ?: false,
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
    ...
    private fun parseBreadcrumb(scope: MagicUIScope, data: JsonObject) {
        val itemsArray = data["items"]?.jsonArray ?: return
        val items = itemsArray.map { itemElement ->
            val itemObj = itemElement.jsonObject
    ...
    private fun parsePagination(scope: MagicUIScope, data: JsonObject) {
        scope.Pagination(
    ...
    private fun parseTable(scope: MagicUIScope, data: JsonObject) {
    ...
    private fun parseList(scope: MagicUIScope, data: JsonObject) {
        val itemsArray = data["items"]?.jsonArray ?: return
        val items = itemsArray.map { itemElement ->
            val itemObj = itemElement.jsonObject
    ...
    private fun parseAccordion(scope: MagicUIScope, data: JsonObject) {
        val itemsArray = data["items"]?.jsonArray ?: return
        val items = itemsArray.map { itemElement ->
            val itemObj = itemElement.jsonObject
    ...
    private fun parseStepper(scope: MagicUIScope, data: JsonObject) {
    ...
    private fun parseTimeline(scope: MagicUIScope, data: JsonObject) {
        val itemsArray = data["items"]?.jsonArray ?: return
        val items = itemsArray.map { itemElement ->
            val itemObj = itemElement.jsonObject
    ...
    private fun parseCarousel(scope: MagicUIScope, data: JsonObject) {
        val itemsArray = data["items"]?.jsonArray ?: return
        val items = itemsArray.map { itemElement ->
    ...
    private fun parseAvatar(scope: MagicUIScope, data: JsonObject) {
        scope.Avatar(
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
        ) {
    ...
    private fun parseChip(scope: MagicUIScope, data: JsonObject) {
        scope.Chip(
            label = data["label"]?.jsonPrimitive?.content ?: "",
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
    ...
    private fun parseDivider(scope: MagicUIScope, data: JsonObject) {
        scope.Divider(
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
        ) {
    ...
    private fun parsePaper(scope: MagicUIScope, data: JsonObject) {
        scope.Paper(
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
        ) {
    ...
    private fun parseSkeleton(scope: MagicUIScope, data: JsonObject) {
        scope.Skeleton(
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
        ) {
    ...
    private fun parseEmptyState(scope: MagicUIScope, data: JsonObject) {
        scope.EmptyState(
            title = data["title"]?.jsonPrimitive?.content ?: "",
            id = data["id"]?.jsonPrimitive?.content,
            style = parseStyle(data["style"])
    ...
    private fun parseDataGrid(scope: MagicUIScope, data: JsonObject) {
        val columnsArray = data["columns"]?.jsonArray ?: return
        val columns = columnsArray.map { columnElement ->
            val columnObj = columnElement.jsonObject
 ...
 */
class YamlGenerator {
    fun generate(ui: MagicUI): String {
