...
class ContextManager(
    ...
    // Current context state (using models.CommandContext for Phase 1 compatibility)
    private val _currentContext = MutableStateFlow<ModelsCommandContext?>(null)
    val currentContext: Flow<ModelsCommandContext?> = _currentContext.asStateFlow()
    
    // Context providers
    private val contextProviders = mutableListOf<ContextProvider>()
    
    // Context rules (legacy)
    private val contextRules = mutableListOf<LegacyContextRule>()
    ...
    fun updateContext(accessibilityService: AccessibilityService?) {
        ...
        try {
            ...
            // Log context info (using models.CommandContext)
            val contextInfo = if (context != null) {
                "App:${context.packageName ?: "unknown"}, Activity:${context.activityName ?: "none"}"
            } else {
                "null"
            }
            android.util.Log.v(TAG, "Context updated: $contextInfo")
        } catch (e: Exception) {
     ...
     * Uses models.CommandContext
     */
    fun getContextualCommands(
        allCommands: List<CommandDefinition>,
        context: ModelsCommandContext? = _currentContext.value
     ...
     * Uses models.CommandContext
     */
    fun isCommandAvailableInContext(command: CommandDefinition, context: ModelsCommandContext): Boolean {
    ...
    /**
     * Register context rule (legacy)
     */
    fun registerContextRule(rule: LegacyContextRule) {
     ...
     * Returns models.CommandContext (stub version for Phase 1 compatibility)
     */
    private fun buildContext(accessibilityService: AccessibilityService?): ModelsCommandContext? {
     ...
     * Uses models.CommandContext (stub version)
     */
    private fun checkContextRequirement(requirement: String, context: ModelsCommandContext): Boolean {
        return when {
            requirement == "text_input" -> {
     ...
     * Uses models.CommandContext (stub version)
     */
    private fun calculateRelevanceScore(command: CommandDefinition, context: ModelsCommandContext): Float {
        var score = 0f
     ...
     * Uses models.CommandContext (simple data class)
     */
    private fun enhanceContext(
        originalContext: ModelsCommandContext?,
        accessibilityService: AccessibilityService?
    ): ModelsCommandContext? {
        if (originalContext == null || accessibilityService == null) {
            return originalContext
        }

        // Enhance with additional accessibility info
        val enhancedCustomData = originalContext.customData.toMutableMap()
        enhancedCustomData["accessibilityService"] = accessibilityService

        return originalContext.copy(customData = enhancedCustomData)
    }
     ...
     * Uses models.CommandContext (simple data class)
     */
    private fun extractContextParameters(context: ModelsCommandContext?): Map<String, Any> {
        if (context == null) return emptyMap()

        val parameters = mutableMapOf<String, Any>()

        // Extract parameters from simple data class
        ...
        context.screenContent?.let { parameters["screenContent"] = it }

        // Include custom data
        parameters.putAll(context.customData)
...
/**
 * Context rule interface (legacy - use ContextRule.kt for new implementation)
 */
