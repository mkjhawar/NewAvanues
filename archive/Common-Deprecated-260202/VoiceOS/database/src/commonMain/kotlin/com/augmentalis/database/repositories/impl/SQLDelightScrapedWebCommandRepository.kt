/**
 * SQLDelightScrapedWebCommandRepository.kt - SQLDelight implementation of IScrapedWebCommandRepository
 *
...
/**
 * SQLDelight implementation of IScrapedWebCommandRepository.
 */
class SQLDelightScrapedWebCommandRepository(
    ...
    override suspend fun insert(command: ScrapedWebCommandDTO): Long = withContext(Dispatchers.Default) {
        ...
        queries.transactionWithResult {
            database.scrapedWebCommandQueries.selectByElementHash(
                command.elementHash,
                command.domainId
            ).executeAsList().firstOrNull()?.id ?: 0L
        }
    }
    ...
    override suspend fun getById(id: Long): ScrapedWebCommandDTO? = withContext(Dispatchers.Default) {
        // Select by domain returns all, we need to filter - or add a selectById query
        // For now, using existing queries
        null // TODO: Add selectById to .sq file
    }
