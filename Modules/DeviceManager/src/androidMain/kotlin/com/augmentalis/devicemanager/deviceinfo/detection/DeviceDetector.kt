...
object DeviceDetector {
    ...
    private fun detectDisplayCapabilities(context: Context): DisplayCapabilities {
        ...
        return DisplayCapabilities(
            ...
            refreshRate = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                currentRefreshRate(context) ?: 60f
            } else 60f,
    ...
    private fun currentRefreshRate(context: Context): Float? {
        return getDefaultDisplay(context)?.mode?.refreshRate
    }

    /** Supported modes (safe replacement for context.display?.supportedModes) */
    private fun supportedModes(context: Context): Array<Display.Mode> {
        return getDefaultDisplay(context)?.supportedModes ?: emptyArray()
    }

    /** Supported refresh rates (deduped + sorted) */
    private fun supportedRefreshRates(context: Context): List<Float> {
        return supportedModes(context)
            .map { it.refreshRate }
            .distinct()
            .sorted()
    }
