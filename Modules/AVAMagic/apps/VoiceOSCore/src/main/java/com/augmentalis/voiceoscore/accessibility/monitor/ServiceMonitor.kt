...
import com.augmentalis.voiceos.accessibility.ConnectionState
...
class ServiceMonitor(
    ...
    private fun updateNotificationIndicator(state: ConnectionState) {
        try {
            // Get state description and icon indicator
            val (stateText, iconEmoji) = when (state) {
                ConnectionState.CONNECTED -> "Connected" to "🟢"
                ConnectionState.RECOVERING -> "Recovering" to "🟡"
                ConnectionState.DEGRADED -> "Degraded" to "🔴"
                ConnectionState.DISCONNECTED -> "Disconnected" to "⚪"
            }

            // Update the foreground service notification via VoiceOSService
            // TODO: Implement updateNotificationStatus in VoiceOSService
            // service.updateNotificationStatus("$iconEmoji $stateText")

            Log.d(TAG, "Notification indicator updated: $state ($stateText)")
        } catch (e: Exception) {
            Log.e(TAG, "Failed to update notification indicator", e)
        }
    }
