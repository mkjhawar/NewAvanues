...
class CarouselHandler(
    ...
    override suspend fun execute(command: QuantizedCommand, params: Map<String, Any>): HandlerResult {
        ...
        Log.d(TAG, "Processing carousel command: '$commandText'")
        ...
        return try {
            when {
                ...
                commandText == PLAY_COMMAND || commandText == RESUME_COMMAND -> {
                    ...
                    Log.d(TAG, "Unrecognized carousel command: $commandText")
                    HandlerResult.notHandled()
                }
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error processing carousel command: $commandText", e)
