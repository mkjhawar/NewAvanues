    ...
    suspend fun registerAll(registry: IHandlerRegistry): Int {
        ...
        allHandlers.forEach { handler ->
            try {
                ...
                registered++
                Log.d(TAG, "Registered: ${handler::class.simpleName}")
            } catch (e: Exception) {
                Log.e(TAG, "Failed to register ${handler::class.simpleName}", e)
            }
        }
        Log.i(TAG, "Registered $registered/${allHandlers.size} handlers")
        return registered
    }
    ...
    suspend fun unregisterAll(registry: IHandlerRegistry): Int {
        ...
        allHandlers.forEach { handler ->
            try {
                ...
                Log.e(TAG, "Failed to unregister ${handler::class.simpleName}", e)
            }
        }
        return unregistered
    }
    ...
    private suspend fun registerList(
        ...
        handlers.forEach { handler ->
            try {
                ...
                Log.e(TAG, "Failed to register $category/${handler::class.simpleName}", e)
            }
        }
        Log.i(TAG, "Registered $registered/${handlers.size} $category handlers")
        return registered
    }
}
