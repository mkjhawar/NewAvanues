/**
 * ThirdPartyAvidGenerator.kt - Generate stable UUIDs for third-party apps
 * Path: libraries/AvidCreator/src/main/java/com/augmentalis/uuidcreator/thirdparty/ThirdPartyAvidGenerator.kt
 *
 * Author: Manoj Jhawar
 ...
 *
 * ```kotlin
 * val generator = ThirdPartyAvidGenerator(context)
 *
...
class ThirdPartyAvidGenerator(
     ...
     */
    private val cache = ThirdPartyAvidCache()
    ...
    /**
     * Format UUID from fingerprint using compact format
     *
     * Creates formatted UUID string according to compact template:
     * `{reversedPackage}:{version}:{typeAbbrev}:{hash8}`
     *
     * Example: `android.instagram.com:12.0.0:btn:a7f3e2c1`
     *
     * @param fingerprint Accessibility fingerprint
     * @return Formatted UUID string in compact format
     */
    private fun formatUuid(fingerprint: AccessibilityFingerprint): String {
        val hash = fingerprint.generateHash().take(8) // Use 8-char hash for compact format
        val type = fingerprint.getElementType()

        return AvidGenerator.generateCompact(
     ...
     * based on format pattern. Supports both compact and legacy formats.
     *
     * Compact format: `android.instagram.com:12.0.0:btn:a7f3e2c1`
     * Legacy format: `com.instagram.android.v12.0.0.button-a7f3e2c1d4b5`
     *
     * @param uuid UUID string to check
     * @return true if third-party UUID format detected
     */
    fun isThirdPartyAvid(uuid: String): Boolean {
        // Check compact format first (preferred)
        if (AvidGenerator.isCompactApp(uuid)) return true

        // Fallback: Legacy format pattern: package.v{version}.type-hash
        val legacyPattern = Regex("^[a-z][a-z0-9_]*(\\.[a-z][a-z0-9_]*)+\\.v[0-9]+\\.[0-9]+\\.[0-9]+\\.[a-z]+-[a-f0-9]{12}$")
        return legacyPattern.matches(uuid)
    }
     ...
     *
     * Compact: `android.instagram.com:12.0.0:btn:a7f3e2c1`
     * Legacy: `com.instagram.android.v12.0.0.button-a7f3e2c1d4b5`
     *
     * @param uuid Third-party UUID string
     * @return Parsed components or null if invalid format
     */
    fun parseThirdPartyAvid(uuid: String): ThirdPartyAvidComponents? {
        // Try compact format first
        AvidGenerator.parse(uuid)?.let { parsed ->
            // Only return if we have package name and version (app format)
            val pkgName = parsed.packageName ?: return@let
            val ver = parsed.version ?: return@let
            return ThirdPartyAvidComponents(
                packageName = pkgName,
                version = ver,
        ...
        if (!isThirdPartyAvid(uuid)) return null
 ...
 */
data class ThirdPartyAvidComponents(
