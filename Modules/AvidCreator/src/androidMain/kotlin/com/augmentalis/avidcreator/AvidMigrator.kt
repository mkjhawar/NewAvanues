/**
 * AvidMigrator.kt - Migration utilities for VUID format conversion
 *
 ...
 */
package com.augmentalis.avidcreator.migration
 ...
 * ```kotlin
 * // Single migration
 * val compact = AvidMigrator.migrate(legacyVuid, version)
 *
 * // Batch migration
 * val results = AvidMigrator.migrateBatch(legacyVuids) { progress ->
 *     Log.d("Migration", "Progress: ${progress.percent}%")
 * }
 * ```
 */
object AvidMigrator {
    private const val TAG = "AvidMigrator"
    ...
    fun migrate(legacyVuid: String, version: String? = null): String {
        ...
        // Use shared AvidGenerator's migration
        val compactVuid = AvidGenerator.migrateToCompact(legacyVuid, version)
    ...
    fun migrateIfNeeded(vuid: String, version: String? = null): String? {
        return if (AvidGenerator.isCompact(vuid)) {
    ...
    suspend fun migrateBatch(
        ...
        vuids.forEachIndexed { index, vuid ->
            try {
                when {
                    AvidGenerator.isCompact(vuid) -> {
                        skipped.add(vuid)
                    }
                    else -> {
                        val compact = AvidGenerator.migrateToCompact(vuid, version)
    ...
    fun needsMigration(vuid: String): Boolean {
        return !AvidGenerator.isCompact(vuid) && AvidGenerator.isValid(vuid)
    }

    /**
     * Parse VUID into components
     */
    fun parse(vuid: String) = AvidGenerator.parse(vuid)
}
