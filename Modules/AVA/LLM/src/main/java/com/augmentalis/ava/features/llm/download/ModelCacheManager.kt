...
class ModelCacheManager(
        ...
        try {
            ...
            if (modelDir.exists()) {
                ...
                if (deleted) {
                    ...
                    Result.Error(
                        exception = IllegalStateException("Failed to delete model directory"),
     ...
     *
     * @return Result with number of models deleted
     */
    suspend fun clearCache(): Result<Int> = withContext(Dispatchers.IO) {
        try {
     ...
     * @return Result indicating if checksum matches
     */
        ...
        try {
            val modelPath = getModelPath(modelId)
                ?: return@withContext Result.Error(
                    exception = IllegalStateException("Model not found"),
     ...
     * @return Result with number of models cleaned up
     */
    suspend fun cleanupOldModels(maxAgeMs: Long): Result<Int> = withContext(Dispatchers.IO) {
        try {
