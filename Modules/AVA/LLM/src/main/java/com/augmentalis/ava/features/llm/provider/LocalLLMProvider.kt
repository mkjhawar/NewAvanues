...
import com.augmentalis.ava.features.llm.alc.inference.GGUFInferenceStrategy
...
import com.augmentalis.ava.features.llm.alc.tokenizer.HuggingFaceTokenizer
...
import com.augmentalis.ava.features.llm.config.DeviceModelSelector
import com.augmentalis.ava.features.llm.config.DeviceProfile
import com.augmentalis.ava.features.llm.config.ModelConfiguration
import com.augmentalis.ava.features.llm.config.ConfigurationType
...
class LocalLLMProvider(
    ...
    override suspend fun initialize(config: LLMConfig): Result<Unit> = modelMutex.withLock {
        return@withLock try {
            ...
            if (isLiteRT) {
                ...
                try {
                    ...
                    val strategy = com.augmentalis.ava.features.llm.alc.inference.LiteRTInferenceStrategy(context, modelDir.absolutePath)
    ...
    private fun chatWithGguf(
        ...
        try {
            ...
            val params = com.augmentalis.ava.features.llm.alc.models.GenerationParams(
                temperature = options.temperature,
                topP = options.topP,
                topK = 40,
     ...
     *
     * @return Result.Success if reloaded, Result.Error if no previous config or failed
     */
    suspend fun reloadModel(): Result<Unit> {
        val config = currentConfig
            ?: return Result.Error(
                exception = IllegalStateException("No previous configuration"),
    ...
    suspend fun initializeWithDeviceOptimization(
        ...
        return try {
            ...
            val config = deviceModelSelector.getRecommendedConfiguration(device, preferMultilingual)
                ?: return Result.Error(
                    exception = IllegalStateException("No compatible models installed"),
     ...
     * @return Result with new configuration, or error if not available
     */
    suspend fun switchToConfiguration(configId: String): Result<ModelConfiguration> {
        return try {
            ...
            val config = configs.find { it.id == configId }
                ?: return Result.Error(
                    exception = IllegalArgumentException("Configuration not found: $configId"),
            ...
            if (!config.runtimeAvailable) {
                return Result.Error(
                    exception = UnsupportedOperationException("Runtime not available"),
            ...
            if (!deviceModelSelector.isConfigurationAvailable(config)) {
                return Result.Error(
                    exception = java.io.FileNotFoundException("Model not installed"),
