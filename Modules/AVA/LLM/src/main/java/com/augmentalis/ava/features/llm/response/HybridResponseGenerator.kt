...
class HybridResponseGenerator(
    ...
    private val cloudLLMProvider: com.augmentalis.ava.features.llm.provider.CloudLLMProvider? = null,
    ...
    override suspend fun generateResponse(
        ...
        if (useCloudLLM && cloudLLMProvider != null) {
            ...
            try {
                ...
                val messages = listOf(
                    com.augmentalis.ava.features.llm.domain.ChatMessage(
                        role = com.augmentalis.ava.features.llm.domain.MessageRole.USER,
                        content = userMessage
                    )
                )

                cloudLLMProvider.chat(messages, com.augmentalis.ava.features.llm.domain.GenerationOptions())
                    .catch { e ->
                    ...
                    .collect { llmResponse ->
                        when (llmResponse) {
                            is com.augmentalis.ava.features.llm.domain.LLMResponse.Streaming -> {
                                emit(ResponseChunk.Text(llmResponse.chunk))
                                hasEmitted = true
                            }
                            is com.augmentalis.ava.features.llm.domain.LLMResponse.Complete -> {
                            ...
                            is com.augmentalis.ava.features.llm.domain.LLMResponse.Error -> {
