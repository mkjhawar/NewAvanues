...
package com.augmentalis.ava.features.llm.alc.language
...
class LanguagePackManager(
     ...
     */
    suspend fun getAvailableLanguages(): Result<List<LanguagePack>> = withContext(Dispatchers.IO) {
        try {
     ...
     */
    suspend fun getLanguagePackInfo(languageCode: String): Result<LocalizedLanguagePack> {
        val availableResult = getAvailableLanguages()
        if (availableResult is Result.Error) {
            return Result.Error(availableResult.exception, availableResult.message)
        }

        val pack = (availableResult as? Result.Success)?.data
            ?.find { it.code == languageCode }
            ?: return Result.Error(
                exception = IllegalArgumentException("Language not found: $languageCode"),
                message = "Language not found: $languageCode"
            )

        return Result.Success(pack.withLocalizationInfo(context))
    }
    ...
    suspend fun downloadLanguagePack(
        ...
        try {
            ...
            if (availableSpace < pack.sizeBytes) {
                return@withContext Result.Error(
                    exception = IllegalStateException("Insufficient storage"),
     ...
     */
    suspend fun deleteLanguagePack(languageCode: String): Result<Unit> = withContext(Dispatchers.IO) {
        if (languageCode == "en") {
            return@withContext Result.Error(
                exception = IllegalArgumentException("Cannot delete base language"),
        ...
        return@withContext try {
            ...
            if (langDir.exists()) {
                ...
                if (deleted) {
                    ...
                    Result.Error(
                        exception = java.io.IOException("Failed to delete directory"),
