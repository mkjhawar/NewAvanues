package com.augmentalis.ava.features.llm.cache
...
class TokenCacheManager(
    ...
    suspend fun getOrTokenize(
        ...
        when (result) {
            is Result.Success -> {
                ...
                val newCount = (tokenCacheRepository.getCacheCount() as? Result.Success)?.data ?: 0
                result.data
            }
            is Result.Error -> {
    ...
    suspend fun getStats(): TokenCacheStats {
        val count = (tokenCacheRepository.getCacheCount() as? Result.Success)?.data ?: 0
