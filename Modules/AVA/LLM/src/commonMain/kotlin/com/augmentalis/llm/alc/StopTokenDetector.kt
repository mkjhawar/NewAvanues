...
object StopTokenDetector {
    ...
    private const val TAG = "StopTokenDetector"
    ...
    fun getStopTokens(modelId: String): Set<Int> {
        val stopTokens = when {
            ...
            modelId.contains("mistral", ignoreCase = true) -> {
                ...
                Logger.w(TAG, "Unknown model: $modelId, using default stop tokens")
                setOf(1, 2) // </s> variants
            }
        }

        Logger.d(TAG, "Stop tokens for $modelId: $stopTokens")
        return stopTokens
    }
    ...
    fun getModelTokenInfo(modelId: String): ModelTokenInfo {
        return when {
            ...
            modelId.contains("mistral", ignoreCase = true) -> {
                ...
                Logger.w(TAG, "Unknown model: $modelId, using default token info")
                ModelTokenInfo(
                    modelId = modelId,
                    eosTokenId = 2,
                    bosTokenId = 1,
