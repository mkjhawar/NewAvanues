...
public class Module extends TVMObject {
   ...
   * @param fmt The format of the file (DEPRECATED in TVM v0.22.0+, ignored)
   *            Format is now auto-detected from file content/extension.
   * @return The loaded module
   */
  public static Module load(String path, String fmt) {
    // TVM v0.22.0 FFI change: ffi.ModuleLoadFromFile now takes only 1 argument (path)
    // The format parameter is no longer used - format is auto-detected
    // Keeping this signature for backwards compatibility but ignoring fmt
    TVMValue ret = getApi("ffi.ModuleLoadFromFile").pushArg(path).invoke();
    return ret.asModule();
  }
