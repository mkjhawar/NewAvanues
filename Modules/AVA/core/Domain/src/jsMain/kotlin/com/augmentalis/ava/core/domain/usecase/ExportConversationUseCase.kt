...
/**
 * JS/Web implementation of conversation export use case
 * Uses Kotlin/JS Date for timestamp formatting
 */
actual class ExportConversationUseCase actual constructor(
    ...
    private fun exportToJson(
        ...
        val filename = "ava_conversation_${conversation.id}_${Date.now().toLong()}.json"
        return ExportResult(
            content = json,
            filename = filename,
    ...
    private fun exportAllToJson(
        ...
        val json = buildString {
            append("{\n")
            append("  \"export_date\": ${escapeJson(formatTimestamp(Date.now().toLong(), false))},\n")
        ...
        val filename = "ava_all_conversations_${Date.now().toLong()}.json"
        return ExportResult(
            content = json,
            filename = filename,
            mimeType = "application/json"
    ...
    private fun exportToCsv(
        ...
        val filename = "ava_conversation_${conversation.id}_${Date.now().toLong()}.csv"
        return ExportResult(content = csv.toString(), filename = filename, mimeType = "text/csv")
    }
    ...
    private fun exportAllToCsv(
        ...
        val filename = "ava_all_conversations_${Date.now().toLong()}.csv"
        return ExportResult(content = csv.toString(), filename = filename, mimeType = "text/csv")
    }

    private fun formatTimestamp(timestamp: Long, anonymize: Boolean): String {
        if (anonymize) return "offset_$timestamp"
