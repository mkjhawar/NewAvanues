...
package com.augmentalis.ava.core.common.ava3
...
class AVA3Decoder {
     ...
     *
     * Useful for TVM library loading which requires file path.
     *
     * @param context Android context
     * @param encodedPath Path to .ava3 file
     * @param outputPath Output path for decoded file
     ...
     *
     * Automatically generates output path in app cache.
     *
     * @param context Android context
     * @param encodedPath Path to .ava3 file
     ...
     *
     * @param encodedPath Path to .ava3 file
     * @return true if file is valid AVA 3.0 format
     */
     ...
     * @return true if file has .ava3 extension or AVA3 magic
     */
    ...
    private fun deriveScramblePattern(key: ByteArray, blockIndex: Int): ByteArray {
        ...
        // Repeat pattern to cover block size
    ...
    private fun deriveNonce(key: ByteArray, blockIndex: Int): ByteArray {
        ...
        return digest.digest(nonceMaterial).copyOf(16)  // AES needs 16-byte IV
    }
    ...
    private fun byteUnshuffle(data: ByteArray, blockIndex: Int, key: ByteArray): ByteArray {
        ...
        // Use same seeded shuffle as encoder
    ...
    private fun decodeBlock(data: ByteArray, blockIndex: Int, key: ByteArray): ByteArray {
        ...
        // Step 3: XOR unscramble
        val pattern = deriveScramblePattern(key, blockIndex)
        return xorData(unshuffled, pattern)
    }

    private fun decompress(data: ByteArray): ByteArray {
        // For Android, use built-in compression
        // Note: This should match the compression used by encoder
        // Python uses ZSTD, Swift uses LZFSE - need to handle both

        // Try to detect and decompress
        // For now, assume data is not compressed or use simple approach
        return try {
            java.util.zip.Inflater().run {
                setInput(data)
                val output = ByteArray(data.size * 4)  // Estimate
                val size = inflate(output)
                end()
                output.copyOf(size)
            }
        } catch (e: Exception) {
            // If decompression fails, data may not be compressed
