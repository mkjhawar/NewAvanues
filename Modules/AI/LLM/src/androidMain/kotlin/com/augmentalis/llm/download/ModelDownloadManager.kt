...
class ModelDownloadManager(
     ...
     */
    suspend fun pauseDownload(modelId: String): LLMResult<Unit> = withContext(Dispatchers.IO) {
        try {
            downloadMutex.withLock {
                val job = activeDownloads[modelId]
                    ?: return@withContext LLMResult.Error(
                        message = "No active download for: $modelId",
                        cause = IllegalStateException("No active download")
                    )
     ...
     * @return LLMResult indicating success or failure
     */
    suspend fun resumeDownload(modelId: String): LLMResult<Unit> = withContext(Dispatchers.IO) {
        try {
            downloadMutex.withLock {
                val job = activeDownloads[modelId]
                    ?: return@withContext LLMResult.Error(
     ...
     * @return LLMResult indicating success or failure
     */
    suspend fun cancelDownload(modelId: String): LLMResult<Unit> = withContext(Dispatchers.IO) {
        try {
            downloadMutex.withLock {
                val job = activeDownloads[modelId]
                    ?: return@withContext LLMResult.Error(
