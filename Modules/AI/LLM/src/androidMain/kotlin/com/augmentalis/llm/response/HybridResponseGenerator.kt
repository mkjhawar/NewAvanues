...
class HybridResponseGenerator(
     ...
     * @return LLMResult indicating LLM init status (templates always work)
     */
        ...
        if (useCloudLLM && cloudLLMProvider != null) {
            ...
            try {
                ...
                cloudLLMProvider.chat(messages, com.augmentalis.llm.domain.GenerationOptions())
                    ...
                    .collect { llmResponse ->
                        when (llmResponse) {
                            is com.augmentalis.llm.LLMResponse.Streaming -> {
                                emit(ResponseChunk.Text(llmResponse.chunk))
                                hasEmitted = true
                            }
                            is com.augmentalis.llm.LLMResponse.Complete -> {
                            ...
                            is com.augmentalis.llm.LLMResponse.Error -> {
                                cloudError = Exception(llmResponse.message)
                            }
                        }
                    }
