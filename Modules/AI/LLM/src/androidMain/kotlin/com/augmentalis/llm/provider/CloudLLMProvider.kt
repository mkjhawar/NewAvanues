...
class CloudLLMProvider(
    ...
    override suspend fun initialize(config: LLMConfig): LLMResult<Unit> = mutex.withLock {
        return@withLock try {
            ...
            val initResults = mutableListOf<Pair<ProviderType, LLMResult<Unit>>>()
            ...
            for (providerType in PROVIDER_PRIORITY) {
                ...
                val apiKey = when (apiKeyResult) {
                    is LLMResult.Success -> apiKeyResult.data
                    is LLMResult.Error -> {
            ...
            if (activeProviders.isEmpty()) {
                return LLMResult.Error(
                    ...
                    cause = IllegalStateException("No cloud providers available")
                )
            }
    ...
    override suspend fun checkHealth(): LLMResult<ProviderHealth> {
        ...
        val healthyCount = healthChecks.count { (_, result) ->
            result is LLMResult.Success && result.data.status == HealthStatus.HEALTHY
        }
