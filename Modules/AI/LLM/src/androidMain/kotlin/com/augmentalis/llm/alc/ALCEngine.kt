...
import com.augmentalis.llm.alc.loader.TVMModelLoader
...
class ALCEngine(
     ...
     */
    suspend fun initialize(): LLMResult<Unit> {
     ...
     */
    suspend fun switchLanguage(languageCode: String): LLMResult<Unit> = mutex.withLock {
        try {
            ...
            if (!languagePackManager.isLanguageInstalled(languageCode)) {
                return LLMResult.Error(
                    ...
                    cause = LanguagePackNotInstalledException(languageCode)
                )
            }
