...
class HuggingFaceModelDownloader(
     ...
     */
    suspend fun copyFromLocalSource(config: ModelDownloadConfig): LLMResult<Unit> = withContext(Dispatchers.IO) {
        val localPath = config.localSourcePath
            ?: return@withContext LLMResult.Error(
                message = "No local source path provided",
                cause = IllegalArgumentException("localSourcePath not set")
            )
        ...
        try {
            ...
            if (!sourceFile.exists()) {
                return@withContext LLMResult.Error(
                    ...
                    cause = java.io.FileNotFoundException("Source not found: $localPath")
                )
            }
    ...
    fun downloadModel(config: ModelDownloadConfig): Flow<DownloadProgress> = flow {
        ...
        if (config.localSourcePath != null) {
            ...
            if (result is LLMResult.Success) return@flow
        }
