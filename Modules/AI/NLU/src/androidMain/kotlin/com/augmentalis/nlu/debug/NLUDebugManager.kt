...
object NLUDebugManager {
    ...
    suspend fun reloadFromAvaSources(context: Context): Result<Int> = withContext(Dispatchers.IO) {
        try {
            ...
            nluLogInfo(TAG, "=== NLU Data Reload Complete ===")
            nluLogInfo(TAG, "Loaded $count examples from .ava files")

            Result.success(count)
        } catch (e: Exception) {
            nluLogError(TAG, "Failed to reload NLU data: ${e.message}", e)
            Result.failure(e)
        }
    }
            ...
            if (coreDir.exists()) {
                coreDir.listFiles()?.forEach { file ->
                    if (file.extension == "ava") {
                        ...
                        nluLogDebug(TAG, "Found .ava file: ${file.name} (${file.length()} bytes)")
                    }
                }
            } else {
                nluLogWarn(TAG, "Core directory not found: ${coreDir.absolutePath}")
            }

            nluLogInfo(TAG, "Found ${avaFiles.size} .ava files")
            avaFiles
        } catch (e: Exception) {
            nluLogError(TAG, "Failed to verify .ava files: ${e.message}", e)
            emptyList()
        }
    }
}
