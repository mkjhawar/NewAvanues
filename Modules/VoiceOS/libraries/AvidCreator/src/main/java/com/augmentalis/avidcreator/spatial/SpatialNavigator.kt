package com.augmentalis.avidcreator.spatial
...
class SpatialNavigator(private val registry: AvidRegistry) {
     ...
     */
    private fun navigateNext(sourceElement: AvidElement): NavigationResult {
     ...
     */
    private fun navigatePrevious(sourceElement: AvidElement): NavigationResult {
        val currentIndex = sourceElement.position?.index ?: 0
        val candidates = registry.getEnabledElements()
            .filter { element -> element.position?.let { it.index < currentIndex } == true }
