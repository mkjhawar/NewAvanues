...
import com.augmentalis.learnapp.overlays.LoginPromptAction
import com.augmentalis.learnapp.overlays.LoginPromptConfig
import com.augmentalis.learnapp.overlays.LoginPromptOverlay
...
class LearnAppIntegration private constructor(
        ...
        scope.launch {
            appLaunchDetector.appLaunchEvents
                ...
                .distinctUntilChanged { old, new ->
                    ...
                    when {
                        old is com.augmentalis.learnapp.detection.AppLaunchEvent.NewAppDetected &&
                        new is com.augmentalis.learnapp.detection.AppLaunchEvent.NewAppDetected ->
    ...
    fun resetLearnedApp(
        ...
        scope.launch {
            ...
            val success = when (result) {
                is com.augmentalis.learnapp.database.repository.RepositoryResult.Success -> true
                is com.augmentalis.learnapp.database.repository.RepositoryResult.Failure -> false
            }

            val message = when (result) {
                is com.augmentalis.learnapp.database.repository.RepositoryResult.Success ->
                    "App '$packageName' reset successfully. Launch the app again to re-learn it."
                is com.augmentalis.learnapp.database.repository.RepositoryResult.Failure ->
    ...
    private fun showLoginPromptOverlay(packageName: String, appName: String) {
        ...
        loginPromptOverlay = LoginPromptOverlay(context, config) { action ->
            when (action) {
