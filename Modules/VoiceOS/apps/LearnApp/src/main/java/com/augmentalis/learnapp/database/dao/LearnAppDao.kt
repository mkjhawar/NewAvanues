...
interface LearnAppDao {
    ...
    /**
     * Insert learned app with IGNORE conflict strategy
     *
     * Used for idempotent operations where existing app + children should be preserved.
     * If app already exists, this method does nothing (no update, no delete).
     *
     * Benefits over REPLACE:
     * - Preserves existing app metadata
     * - Prevents CASCADE delete of exploration_sessions, navigation_edges, screen_states
     * - Safe for concurrent operations
     *
     * @param app Learned app entity to insert
     * @return Row ID if inserted, -1 if conflict (app already exists)
     *
     * @since 1.0.0
     */
    ...
    @Query("DELETE FROM screen_states WHERE package_name = :packageName")
    suspend fun deleteScreenStatesForPackage(packageName: String)

    @Query("DELETE FROM exploration_sessions WHERE package_name = :packageName")
