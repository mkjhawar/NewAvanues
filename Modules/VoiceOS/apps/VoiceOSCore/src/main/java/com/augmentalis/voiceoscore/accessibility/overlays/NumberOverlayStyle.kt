...
enum class AnchorPoint {
    TOP_LEFT,
    TOP_RIGHT,     // Default - most common in UI patterns (like notification badges)
    BOTTOM_LEFT,
    BOTTOM_RIGHT
}
...
enum class BadgeStyle {
    FILLED_CIRCLE,      // Solid filled circle (default)
    OUTLINED_CIRCLE,    // Outlined circle with hollow center
    SQUARE,             // Square badge
    ROUNDED_RECT        // Rounded rectangle badge
}

/**
 * Element voice state for color coding
 */
enum class ElementVoiceState {
    ENABLED_WITH_NAME,   // Green - element has custom command name
    ENABLED_NO_NAME,     // Orange - element is voice-enabled but uses default name
    DISABLED             // Grey - element is not voice-enabled
}
