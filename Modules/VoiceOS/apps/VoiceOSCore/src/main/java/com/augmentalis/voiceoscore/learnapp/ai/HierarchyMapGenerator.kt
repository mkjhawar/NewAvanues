...
class HierarchyMapGenerator(
    ...
    private fun generateDisplayName(screen: ScreenContextDTO): String {
        ...
        // Use local variables to allow smart cast
        val windowTitle = screen.windowTitle
        val activityName = screen.activityName
        val screenType = screen.screenType
        return when {
            !windowTitle.isNullOrBlank() -> windowTitle
            !activityName.isNullOrBlank() -> cleanActivityName(activityName)
            !screenType.isNullOrBlank() -> screenType
