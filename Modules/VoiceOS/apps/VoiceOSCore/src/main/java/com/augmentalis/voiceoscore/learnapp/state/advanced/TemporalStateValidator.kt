...
class TemporalStateValidator(
    ...
    fun validateState(
        ...
        val transientThreshold = developerSettings.getTransientStateDurationMs()
        if (currentDuration < transientThreshold && currentDuration > 0L) {
            return TemporalValidationResult(
                isValid = false,
                confidenceAdjustment = -PENALTY_TRANSIENT,
                reason = "Transient state: ${currentDuration}ms (threshold: ${transientThreshold}ms)"
            )
        }
        ...
        val stableThreshold = developerSettings.getStableStateDurationMs()
        if (currentDuration >= stableThreshold) {
            return TemporalValidationResult(
                isValid = true,
    ...
    fun detectFlickerPatterns(currentTime: Long = System.currentTimeMillis()): List<FlickerPattern> {
        ...
        val flickerWindow = developerSettings.getFlickerDetectionWindowMs()
        val minOccurrences = developerSettings.getMinFlickerOccurrences()
        val recentHistory = durationHistory.filter {
            currentTime - it.startTime <= flickerWindow
        }

        if (recentHistory.size < minOccurrences) {
            return patterns
        }
        ...
        changePatterns.forEach { (change, count) ->
            if (count >= minOccurrences) {
                ...
                if (reverseCount >= minOccurrences) {
