/**
 * LearnAppCoreAdapter.kt - Migration bridge from LearnAppCore to VoiceOSCore
 *
 * Copyright (C) Manoj Jhawar/Aman Jhawar, Intelligent Devices LLC
 * Author: VOS4 Development Team
 * Created: 2026-01-06
 *
 * This adapter bridges the old LearnAppCore API to the new VoiceOSCore API,
 ...
 *
 * ## New API (VoiceOSCore):
 * - Uses AVID (Avanues ID) for element identification
...
/**
 * New ElementProcessingResult for VoiceOSCore.
 */
...
/**
 * LearnAppCoreAdapter - Bridge between old LearnAppCore and new VoiceOSCore.
 *
 * ## Usage:
 *
 ...
 * ```
 *
 * ### Migrating UUIDs to AVIDs:
 * ```kotlin
 * val legacyUuid = "com.app.button-abc123"
 * val avid = LearnAppCoreAdapter.migrateUuidToAvid(legacyUuid)
 * ```
 *
...
class LearnAppCoreAdapter {
     ...
     *
     * @deprecated Use ElementFingerprint.generate() instead to create AVID
     */
    @Deprecated(
        message = "Use ElementFingerprint.generate() for new code to create AVID",
    ...
    companion object {
        ...
        /**
         * Migrate a legacy UUID to new AVID (Avanues ID) format.
         *
         * @param legacyUuid The legacy UUID string
         * @return AVID string, or null if migration failed
         */
        ...
        /**
         * Convert legacy ElementProcessingResult to new format with AVID.
         */
        fun convertProcessingResult(legacy: LegacyElementProcessingResult): NewElementProcessingResult {
            val avid = if (legacy.uuid.isNotEmpty()) {
                migrateUuidToAvid(legacy.uuid) ?: legacy.uuid
            } else null
