...
private class CommandMetric(val phrase: String) {
    ...
    fun record(result: CommandExecutionResult) {
        count++
        totalDurationMs += result.executionTime
        lastDurationMs = result.executionTime
        lastTimestamp = result.command?.timestamp ?: 0L

        if (result.executionTime < minDurationMs) minDurationMs = result.executionTime
        if (result.executionTime > maxDurationMs) maxDurationMs = result.executionTime
