/**
 * DesktopLlmFallbackHandler.kt - Desktop LLM Fallback Handler Stub
 *
 ...
 * TODO: Implement using llama.cpp JNI bindings or Ollama for LLM fallback.
 */
...
/**
 * Desktop stub implementation of [ILlmFallbackHandler].
 *
 ...
 */
class DesktopLlmFallbackHandler(
    ...
    override suspend fun initialize(): Result<Unit> {
        ...
        initialized = true
        println("[DesktopLlmFallbackHandler] Stub initialized - LLM fallback not available on Desktop yet")
    ...
    override suspend fun handleLowConfidence(
        ...
        return FallbackResult.NoMatch(
            reason = "LLM fallback not available on Desktop",
    ...
    override suspend fun handleAmbiguous(
        ...
        return if (bestCandidate != null) {
            FallbackResult.Clarified(
                ...
                source = "heuristic",
                explanation = "Selected highest confidence candidate (LLM not available on Desktop)"
            )
        } else {
    ...
    override suspend fun generateAction(
        ...
        return FallbackResult.NoMatch(
            reason = "Action generation not available on Desktop",
            attemptedProviders = emptyList()
        )
    }
    ...
    /**
     * Returns false because cloud LLM is not implemented on Desktop.
     */
