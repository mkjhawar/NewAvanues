...
fun NoteEditor(
    ...
    Column(modifier = modifier.fillMaxSize()) {
        ...
        Row(
            ...
            Row {
                IconButton(onClick = {}) { Icon(Icons.Default.FormatBold, "Bold", tint = colors.textPrimary) }
                IconButton(onClick = {}) { Icon(Icons.Default.FormatItalic, "Italic", tint = colors.textPrimary) }
            }
            Row {
                IconButton(onClick = onTakePhoto) { Icon(Icons.Default.CameraAlt, "Photo", tint = colors.textPrimary) }
                IconButton(onClick = onAttachFile) { Icon(Icons.Default.AttachFile, "Attach", tint = colors.textPrimary) }
        ...
        Column(Modifier.weight(1f).fillMaxWidth().verticalScroll(rememberScrollState()).padding(16.dp)) {
            BasicTextField(
                ...
                textStyle = TextStyle(color = colors.textPrimary, fontSize = 24.sp, fontWeight = FontWeight.Bold),
                cursorBrush = SolidColor(colors.primary),
                decorationBox = { inner -> Box { if (titleField.text.isEmpty()) Text("Title", color = colors.textPrimary.copy(alpha = 0.4f), fontSize = 24.sp, fontWeight = FontWeight.Bold); inner() } },
                modifier = Modifier.fillMaxWidth()
            )
            Spacer(Modifier.height(16.dp))
            BasicTextField(
                value = contentField, onValueChange = { contentField = it },
                textStyle = TextStyle(color = colors.textPrimary.copy(alpha = 0.9f), fontSize = 16.sp, lineHeight = 24.sp),
                cursorBrush = SolidColor(colors.primary),
                decorationBox = { inner -> Box { if (contentField.text.isEmpty()) Text("Start typing or dictate...", color = colors.textPrimary.copy(alpha = 0.3f), fontSize = 16.sp); inner() } },
                modifier = Modifier.fillMaxWidth().height(400.dp)
            )
            if (attachments.isNotEmpty()) {
                Spacer(Modifier.height(16.dp))
                Text("Attachments", color = colors.textPrimary, fontWeight = FontWeight.SemiBold, fontSize = 14.sp)
        ...
        Row(Modifier.fillMaxWidth().background(colors.surface.copy(alpha = 0.5f)).padding(horizontal = 16.dp, vertical = 6.dp), horizontalArrangement = Arrangement.SpaceBetween) {
            ...
            Text("${contentField.text.length} chars", color = colors.textPrimary.copy(alpha = 0.5f), fontSize = 12.sp)
        }
    }
}
