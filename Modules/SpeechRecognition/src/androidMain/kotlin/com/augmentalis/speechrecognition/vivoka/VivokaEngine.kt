/**
 * VivokaEngine.kt - SOLID refactored Vivoka VSDK speech recognition engine
 *
...
class VivokaEngine(
    ...
    suspend fun initialize(speechConfig: SpeechConfig): Boolean {
        ...
        try {
            ...
            errorListener?.invoke(SpeechError(500, "Initialization failed: ${e.message}"))
            return false
        }
    }
    ...
    private suspend fun prepareLanguageModels(speechConfig: SpeechConfig): Boolean {
        return try {
            ...
            errorListener?.invoke(SpeechError(500, "Language preparation failed: ${e.message}"))
            false
        }
    }
    ...
    private suspend fun downloadAndMergeLanguageModel(languageCode: String): String? {
        return try {
            ...
            val configFile = firebaseRepo.getLanguageResource(
                ...
                when (status) {
                    FileStatus.Completed -> {
                        ...
                        // Notify observers (using code 0 for success status)
                        errorListener?.invoke(SpeechError(0, "Language model downloaded successfully"))
                    }
                    ...
                    is FileStatus.Downloading -> {
                        ...
                        // Provide progress feedback to UI (using code -1 for progress update)
                        errorListener?.invoke(SpeechError(-1, "Downloading $languageCode: $progress%"))
                    }
                    ...
                    FileStatus.Extracting -> {
                        ...
                        errorListener?.invoke(SpeechError(-1, "Extracting $languageCode model files..."))
                    }
                    ...
                    FileStatus.Initialization -> {
                        ...
                        errorListener?.invoke(SpeechError(-1, "Initializing $languageCode model..."))
                    }
                    ...
                    is FileStatus.Error -> {
                        ...
                        errorListener?.invoke(SpeechError(503, "Download failed: $errorType error"))
                    }
                }
            }
            ...
            if (configFile.isNullOrBlank()) {
                ...
                voiceStateManager.setVoiceEnabled(false)
                voiceStateManager.downloadingModels(false)
                errorListener?.invoke(SpeechError(503, errorMsg))
            ...
            if (mergedConfigPath == null) {
                ...
                voiceStateManager.setVoiceEnabled(false)
                errorListener?.invoke(SpeechError(500, errorMsg))
            ...
            voiceStateManager.downloadingModels(false)
            errorListener?.invoke(SpeechError(500, "Download failed: ${e.message}"))
            null
        }
    }
