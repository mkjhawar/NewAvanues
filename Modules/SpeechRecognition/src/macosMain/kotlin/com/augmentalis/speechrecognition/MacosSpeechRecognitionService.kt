/**
 * MacosSpeechRecognitionService.kt - macOS speech recognition using Apple SFSpeechRecognizer
 *
 * Copyright (C) Manoj Jhawar/Aman Jhawar, Intelligent Devices LLC
 *
 * macOS implementation using SFSpeechRecognizer (macOS 10.15+).
 * Same Speech framework API as iOS, with these differences:
 * - No AVAudioSession (macOS doesn't require audio session category configuration)
 * - On-device recognition may have different availability
 *
 * Supports: streaming recognition, partial results, multi-locale,
 * on-device recognition (when available), alternatives.
 */
...
/**
 * macOS implementation of SpeechRecognitionService using SFSpeechRecognizer.
 *
 * Uses AVAudioEngine for microphone input and SFSpeechAudioBufferRecognitionRequest
 * for streaming speech-to-text. Supports on-device recognition,
...
class MacosSpeechRecognitionService : SpeechRecognitionService {
    ...
    companion object {
        private const val TAG = "MacosSpeechService"
    ...
    override suspend fun initialize(config: SpeechConfig): Result<Unit> {
        return try {
            ...
            if (speechRecognizer == null) {
                ...
                _errorFlow.emit(SpeechError.modelError(
                    "Speech recognition unavailable for ${config.language}. " +
            ...
            logInfo(TAG, "Initialized with locale: ${config.language}, " +
     ...
     *
     * On macOS, the user will see a system prompt for microphone + speech recognition
     * permissions the first time this is called.
     */
        ...
        suspendCancellableCoroutine<Unit> { cont ->
            SFSpeechRecognizer.requestAuthorization { rawStatus ->
                ...
                when (status) {
                    ...
                    AUTH_DENIED -> {
                        logError(TAG, "Speech recognition denied — check System Settings > Privacy > Speech Recognition")
                        scope.launch {
                            _errorFlow.emit(SpeechError.permissionError(
                                "Speech recognition denied. Enable in System Settings > Privacy & Security > Speech Recognition."
                            ))
                        }
                        cont.resume(Unit) {}
                    }
                    AUTH_RESTRICTED -> {
                        logError(TAG, "Speech recognition restricted on this Mac")
                        scope.launch {
    ...
    override suspend fun startListening(): Result<Unit> {
        return try {
            ...
            // macOS does NOT require AVAudioSession configuration
            // (unlike iOS which needs setCategory + setActive)
            ...
            recognitionTask = speechRecognizer?.recognitionTaskWithRequest(request) { result, error ->
                ...
                result?.let { speechResult ->
                    ...
                    // Get confidence from last segment
