...
import com.augmentalis.Avanues.web.universal.actions.WebActionExtractor
...
class AndroidWebViewController(
    ...
    // Lazy-initialized action extractor (created when first needed)
    private var actionExtractor: WebActionExtractor? = null

    private fun getOrCreateActionExtractor(): WebActionExtractor? {
        val view = webView ?: return null
        if (actionExtractor == null) {
            actionExtractor = WebActionExtractor(view)
        }
    ...
    override suspend fun extractWebActions(): String? {
        val extractor = getOrCreateActionExtractor()
            ?: return null

        val result = extractor.extractAll()
        return result?.let {
            kotlinx.serialization.json.Json.encodeToString(
                com.augmentalis.Avanues.web.universal.actions.WebActionsResult.serializer(),
                it
            )
        }
    }

    override suspend fun getVoiceCommands(): String? {
        val extractor = getOrCreateActionExtractor()
            ?: return null
        ...
        return if (commands.isNotEmpty()) {
            kotlinx.serialization.json.Json.encodeToString(
                kotlinx.serialization.builtins.ListSerializer(
                    com.augmentalis.Avanues.web.universal.actions.VoiceCommand.serializer()
                ),
                commands
            )
        } else {
    ...
    override suspend fun clickByVoiceCommand(command: String): ActionResult {
        val extractor = getOrCreateActionExtractor()
            ?: return ActionResult.error("No active WebView")

        val result = extractor.clickByCommand(command)
        return if (result.success) {
            ActionResult.success("Clicked ${result.label ?: result.command ?: command}")
        } else {
            ActionResult.error(result.error ?: "Click failed")
        }
    }

    override suspend fun typeText(text: String, selector: String?): ActionResult {
        val extractor = getOrCreateActionExtractor()
            ?: return ActionResult.error("No active WebView")

        val result = extractor.typeText(text, selector)
        return if (result.success) {
            ActionResult.success("Typed text")
        } else {
            ActionResult.error(result.error ?: "Type failed")
        }
    }

    /**
     * Reset action extractor when page changes
     * Call this after navigation to reload the action extraction library
     */
    fun resetActionExtractor() {
        actionExtractor?.reset()
    }
}
