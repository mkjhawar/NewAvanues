...
class WebAvanueVoiceOSBridge(private val webView: WebView) {
    ...
    /**
     * One-time nonce for the current scrape request sent to the page.
     *
     * Generated by [generateScrapeNonce] before injecting any JS that is expected
     * to call back via [VoiceOSInterface.sendScrapeResult]. The nonce is validated
     * inside [VoiceOSInterface.sendScrapeResult] and cleared after first use so
     * a compromised page cannot replay or fabricate scrape payloads.
     */
    @Volatile
    private var scrapeNonce: String = ""

    /**
     * Generate a one-time nonce for a pending [VoiceOSInterface.sendScrapeResult] call.
     *
     * Call this before injecting JavaScript that is expected to call back via
     * [VoiceOSInterface.sendScrapeResult]. Pass the returned nonce to that JS so it
     * can include it in the call. The nonce is a random UUID and is valid only for
     * the next [VoiceOSInterface.sendScrapeResult] invocation — it is consumed on use.
     *
     * @return The nonce string the JS caller must pass as the second argument to
     *         `VoiceOSBridge.sendScrapeResult(json, nonce)`.
     */
    fun generateScrapeNonce(): String {
    ...
    inner class VoiceOSInterface {
        ...
        /**
         * Receive a DOM scrape result pushed from page-side JavaScript.
         *
         * The caller MUST obtain a fresh nonce via [generateScrapeNonce] before
         * injecting the JS and pass it back here as [nonce]. Calls with a missing
         * or mismatched nonce are silently dropped — this prevents malicious page
         * scripts from injecting fake DOM element payloads.
         *
         * The nonce is consumed on first successful match (single-use).
         */
        @JavascriptInterface
        fun sendScrapeResult(jsonResult: String, nonce: String) {
            val expected = scrapeNonce
            if (expected.isEmpty() || nonce != expected) {
                Log.w(TAG, "sendScrapeResult rejected — nonce mismatch")
                return
            }
            // Consume the nonce so a replay call from the same page is rejected.
            scrapeNonce = ""
            try {
