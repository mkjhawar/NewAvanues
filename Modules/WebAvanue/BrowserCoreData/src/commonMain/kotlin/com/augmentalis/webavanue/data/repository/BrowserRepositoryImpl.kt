...
class BrowserRepositoryImpl(
    ...
    // Without this, the StateFlows start empty and tabs/favorites/history are never loaded
    init {
        // Load all data from database synchronously on init
        // This ensures data is available immediately when ViewModels observe the flows
        try {
            ...
            _settings.value = dbSettings?.toDomainModel() ?: BrowserSettings.default()

            println("BrowserRepositoryImpl: Loaded ${_tabs.value.size} tabs, ${_favorites.value.size} favorites, ${_history.value.size} history entries")
        } catch (e: Exception) {
    ...
    override suspend fun updateDownload(download: Download): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            // Use insertDownload with OR REPLACE to update
            queries.insertDownload(download.toDbModel())
            refreshDownloads()
            Result.success(Unit)
        } catch (e: Exception) {
...
private fun Tab.toDbModel() = com.augmentalis.webavanue.data.Tab(
    ...
    parent_tab_id = parentTabId, session_data = sessionData,
...
private fun com.augmentalis.webavanue.data.Tab.toDomainModel() = Tab(
    ...
    position = position.toInt(), parentTabId = parent_tab_id, sessionData = session_data,
...
private fun com.augmentalis.webavanue.data.Browser_settings.toDomainModel() = BrowserSettings(
    ...
    homePage = home_page, newTabPage = BrowserSettings.NewTabPage.valueOf(new_tab_page),
    ...
    dataSaver = data_saver != 0L, autoPlay = BrowserSettings.AutoPlay.valueOf(auto_play),
