...
import com.augmentalis.rag.data.InMemoryRAGRepository
...
class RAGChatEngineIntegrationTest {
     ...
     */
    private class MockEmbeddingProvider : com.augmentalis.rag.embeddings.EmbeddingProvider {
        override val name: String = "MockEmbedding"
        override val dimension: Int = 384

        override suspend fun isAvailable(): Boolean = true

        override suspend fun embed(text: String): Result<com.augmentalis.rag.domain.Embedding.Float32> {
            val hash = text.hashCode()
            val values = FloatArray(dimension) { i ->
                ((hash + i) % 1000) / 1000f
            }
            return Result.success(com.augmentalis.rag.domain.Embedding.Float32(values))
        }

        override suspend fun embedBatch(texts: List<String>): Result<List<com.augmentalis.rag.domain.Embedding.Float32>> {
