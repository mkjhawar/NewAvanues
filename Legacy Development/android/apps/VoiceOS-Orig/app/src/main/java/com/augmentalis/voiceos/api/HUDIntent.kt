/**
 * HUDIntent.kt
 * Path: app/src/main/java/com/augmentalis/voiceos/api/HUDIntent.kt
 * 
 * Created: 2025-01-23
 * Author: VOS4 Development Team
 * Version: 1.0.0
 * 
 * Purpose: System-wide HUD intent definitions
 * Provides public API for all apps to interact with HUD system
 */
...
object HUDIntent {
    ...
    // Intent actions (system-wide namespace)
    const val ACTION_SHOW_HUD = "com.augmentalis.voiceos.ACTION_SHOW_HUD"
    const val ACTION_HIDE_HUD = "com.augmentalis.voiceos.ACTION_HIDE_HUD"
    const val ACTION_TOGGLE_HUD = "com.augmentalis.voiceos.ACTION_TOGGLE_HUD"
    const val ACTION_SHOW_NOTIFICATION = "com.augmentalis.voiceos.ACTION_SHOW_NOTIFICATION"
    const val ACTION_UPDATE_OVERLAY = "com.augmentalis.voiceos.ACTION_UPDATE_OVERLAY"
    const val ACTION_SHOW_VOICE_COMMAND = "com.augmentalis.voiceos.ACTION_SHOW_VOICE_COMMAND"
    const val ACTION_SHOW_DATA_VISUALIZATION = "com.augmentalis.voiceos.ACTION_SHOW_DATA_VISUALIZATION"
    const val ACTION_SET_HUD_MODE = "com.augmentalis.voiceos.ACTION_SET_HUD_MODE"
    const val ACTION_ENABLE_GAZE_TRACKING = "com.augmentalis.voiceos.ACTION_ENABLE_GAZE_TRACKING"
    const val ACTION_DISABLE_GAZE_TRACKING = "com.augmentalis.voiceos.ACTION_DISABLE_GAZE_TRACKING"
    const val ACTION_SET_ACCESSIBILITY_MODE = "com.augmentalis.voiceos.ACTION_SET_ACCESSIBILITY_MODE"
    
    // Language support
    const val EXTRA_LANGUAGE_CODE = "language_code"
    const val EXTRA_AUTO_LOCALIZE = "auto_localize"
    ...
    // Permission for HUD access (system-wide)
    const val PERMISSION_USE_HUD = "com.augmentalis.voiceos.permission.USE_HUD"
    const val PERMISSION_MANAGE_HUD = "com.augmentalis.voiceos.permission.MANAGE_HUD"
    
    /**
     * Create intent to show HUD notification with automatic localization
     */
    fun createShowNotificationIntent(
        ...
        return Intent(ACTION_SHOW_NOTIFICATION).apply {
            ...
            putExtra(EXTRA_AUTO_LOCALIZE, autoLocalize)
            languageCode?.let { putExtra(EXTRA_LANGUAGE_CODE, it) }
        }
    }
    
    /**
     * Create localized notification intent using translation key
     */
    fun createLocalizedNotificationIntent(
        context: Context,
        ...
        val localizedMessage = localization.translate(translationKey, *args)
        
        return Intent(ACTION_SHOW_NOTIFICATION).apply {
            putExtra(EXTRA_NOTIFICATION_MESSAGE, localizedMessage)
            putExtra(EXTRA_NOTIFICATION_DURATION, duration)
            putExtra(EXTRA_NOTIFICATION_POSITION, position)
            putExtra(EXTRA_NOTIFICATION_PRIORITY, priority)
            putExtra(EXTRA_AUTO_LOCALIZE, false) // Already localized
            putExtra(EXTRA_LANGUAGE_CODE, localization.getCurrentLanguage())
        }
    }
