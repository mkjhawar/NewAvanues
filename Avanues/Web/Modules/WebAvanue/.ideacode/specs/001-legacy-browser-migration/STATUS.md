...
### 2025-11-26 - Bug Fix Release v1.1.1 (Deep Root Cause Fixes)
**Permanent fixes for 8 persistent browser issues by addressing root causes:**

| Issue | Root Cause | v1.1.1 Fix |
|-------|------------|------------|
| Tab History Lost | `AndroidView.factory` only runs once, not on tab switch | Added `key(tabId)` to force AndroidView recreation |
| Tabs/Favorites Not Restored | Repository StateFlows started empty, never loaded from DB | Added `init` block to `BrowserRepositoryImpl` |
| Scroll Not Working in Sub-levels | Scroll state persisted across level changes | Added `LaunchedEffect` to reset scroll position |
| Desktop Mode Not Working | Stale WebView reference in WebViewController | Fixed by Issue #1 - correct reference per tab |
| Tab Nav Visible on Fresh Install | Default `tabCount = 1` instead of 0 | Changed default to `tabCount = 0` |
| Keyboard Not Closing | `LocalSoftwareKeyboardController` can return null | Added `focusManager.clearFocus()` fallback |

**Files Changed:**
- `WebViewContainer.android.kt` - `key(tabId)` wrapper around AndroidView
- `BrowserRepositoryImpl.kt` - `init` block loads data from SQLDelight on creation
- `BottomCommandBar.kt` - `LaunchedEffect` resets scroll, default `tabCount = 0`
- `AddressBar.kt` - `dismissKeyboard()` uses both controller and FocusManager

**Commit:** 3d0199b

---

### 2025-11-25 12:00 - Bug Fix Release v1.1.0
**Major bug fix release resolving 9 critical browser issues:**

1. ✅ **Tab History Preserved** - WebViewPool caches WebViews per tab ID, preventing recreation
2. ✅ **Tabs Restore on Startup** - ViewModels wrapped in `remember{}` to persist across recompositions
3. ✅ **Favorites Load on Startup** - Same fix as #2
4. ✅ **Command Bar Scrollable** - Added `horizontalScroll(rememberScrollState())` modifier
5. ✅ **Updated Icons** - Added `compose.materialIconsExtended` for Mic, Laptop, FileDownload, TouchApp
6. ✅ **Desktop Mode Fixed** - Added `webView?.reload()` after user agent change
7. ✅ **Smart Tab Navigation** - Next/Prev buttons conditional on `tabCount >= 2`
8. ✅ **Menu Access** - Added Menu button with `MoreVert` icon to MainCommandBar
9. ✅ **Keyboard Auto-Close** - Added `LocalSoftwareKeyboardController` with `hide()` on search

**New Files:**
- `WebViewPoolManager.kt` - expect/actual for platform-agnostic WebView cleanup

**Modified Files:**
- `build.gradle.kts`, `WebViewContainer.android.kt`, `BrowserApp.kt`, `AddressBar.kt`
- `BottomCommandBar.kt`, `BrowserScreen.kt`, `WebViewContainer.kt`
- `WebViewContainer.desktop.kt`, `WebViewContainer.ios.kt`

**Documentation:**
- Created `docs/develop/webavanue/webavanue-bugfix-release-202511251200.md`
- Updated `docs/webavanue/webavanue-user-manual.md` with v1.1.0 release notes
