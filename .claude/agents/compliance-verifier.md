...
STATUS: ✅ Isolated | ❌ VIOLATION: Cross-repo detected
```
...
if [[ ! "$filename" =~ ^[A-Z][a-zA-Z]+-[A-Z] ]]; then
    echo "❌ VIOLATION: Missing App prefix"
fi

# Year must be single digit
if [[ "$filename" =~ -[0-9]{2}[0-9]{5}- ]]; then
    echo "❌ VIOLATION: Two-digit year (use 5 not 25)"
fi
```
...
|----------|--------|---------|
| Pass | `✅ {rule}` | `✅ Terminal Isolation` |
| Violation | `❌ {rule}: {detail}` | `❌ Naming: Missing App prefix in foo.md` |
| Warning | `⚠️  {rule}: {detail}` | `⚠️  Hardwiring: Possible hardcoded URL` |

**Summary Table:**
```
| Rule | Status | Details |
|------|--------|---------|
| Terminal Isolation | ✅ | ideacode repo |
| Naming | ❌ | 3 files missing App prefix |
| Hardwiring | ⚠️  | 1 possible violation |
| Main Commits | ✅ | On feature/compliance branch |
```
