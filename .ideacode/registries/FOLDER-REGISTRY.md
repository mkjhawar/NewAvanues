...
| Path | Purpose |
|------|---------|
| contextsave/ | Context saves and session handover |
| .claude/ | Claude Code config (NO commands here) |
| .ideacode/ | IDEACODE config |
| .ideacode/living-docs/ | Project memory (LD-*.md) |
| .ideacode/registries/ | File/folder tracking |
| .ideacode/archive/ | Completed work |
| specs/ | Feature specifications and plans |
| docs/ | Documentation |
| ideacode-mcp/ | MCP server source (ideacode repo only) |
| ideacode-plugin/ | Claude Code plugin distribution (ideacode repo only) |
| programming-standards/ | Code standards (ideacode repo only) |
| protocols/ | Workflow protocols (ideacode repo only) |
