...
class VivokaAudio(
    ...
    suspend fun initializePipeline(recognizer: Recognizer): Boolean {
        return try {
            ...
            audioErrorListener?.invoke("Audio pipeline initialization failed: ${e.message}", SpeechError.AUDIO_PIPELINE_ERROR)
            false
        }
    }
    ...
    suspend fun startPipeline(): Boolean {
        return try {
            ...
            audioErrorListener?.invoke("Audio pipeline start failed: ${e.message}", SpeechError.AUDIO_PIPELINE_ERROR)
            false
        }
    }
    ...
    fun startRecording(): Boolean {
        return try {
            ...
            audioErrorListener?.invoke("Audio recording start failed: ${e.message}", SpeechError.AUDIO_PIPELINE_ERROR)
            false
        }
    }
            ...
            coroutineScope.launch {
                ...
                if (!recoverySuccess) {
                    audioErrorListener?.invoke(
                        ...
                        SpeechError.AUDIO_PIPELINE_ERROR
                    )
                }
            }
        } else {
