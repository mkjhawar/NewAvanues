...
object AppActions {
    ...
    class OpenAppAction : BaseAction() {
        ...
        private fun openAppByPackage(context: Context, packageName: String): CommandResult {
            return try {
                val intent = context.packageManager.getLaunchIntentForPackage(packageName)
                if (intent != null) {
                    intent.flags = Intent.FLAG_ACTIVITY_NEW_TASK
                    context.startActivity(intent)
                    createSuccessResult(Command("", "", source = CommandSource.VOICE), "Opened app: $packageName")
                } else {
                    createErrorResult(Command("", "", source = CommandSource.VOICE), ErrorCode.EXECUTION_FAILED, "App not found: $packageName")
                }
            } catch (e: Exception) {
                createErrorResult(Command("", "", source = CommandSource.VOICE), ErrorCode.EXECUTION_FAILED, "Failed to open app: ${e.message}")
            }
        }
        
        private fun openAppByName(context: Context, appName: String): CommandResult {
            return try {
                ...
                if (targetApp != null) {
                    ...
                    createSuccessResult(Command("", "", source = CommandSource.VOICE), "Opened app: $actualName")
                } else {
                    createErrorResult(Command("", "", source = CommandSource.VOICE), ErrorCode.EXECUTION_FAILED, "App not found: $appName")
                }
            } catch (e: Exception) {
                createErrorResult(Command("", "", source = CommandSource.VOICE), ErrorCode.EXECUTION_FAILED, "Failed to open app: ${e.message}")
            }
    ...
    class CloseAppAction : BaseAction() {
        ...
        private fun closeSpecificApp(context: Context, appName: String, command: Command): CommandResult {
            return try {
    ...
    class AppInfoAction : BaseAction() {
        ...
        private fun getCurrentAppInfo(context: Context, command: Command): CommandResult {
            return try {
                val packageManager = context.packageManager
        ...
        private fun getSpecificAppInfo(context: Context, appName: String, command: Command): CommandResult {
            return try {
                val packageManager = context.packageManager
                val intent = Intent(Intent.ACTION_MAIN).apply {
                    addCategory(Intent.CATEGORY_LAUNCHER)
