 ...
 * Created: 2025-09-03
 * 
 * Handles number overlay display for UI element identification,
 * enabling users to interact with elements by voice number commands.
 * 
 * Migrated from Legacy Avenue NumberAction.kt with VOS4 patterns
 */
...
class NumberHandler(
    ...
    override fun execute(
        ...
        return when {
            // Show number overlay
            action.contains("show_numbers", ignoreCase = true) ||
            action.contains("numbers_on", ignoreCase = true) ||
            action.contains("label_elements", ignoreCase = true) ||
            action.contains("number_overlay", ignoreCase = true) -> {
                showNumberOverlay()
            }

            // Hide number overlay
            action.contains("hide_numbers", ignoreCase = true) ||
            action.contains("numbers_off", ignoreCase = true) -> {
                hideNumberOverlay()
            }

            // Toggle number overlay
            action.contains("toggle_numbers", ignoreCase = true) -> {
                toggleNumberOverlay()
            }
    ...
    private fun extractNumberFromCommand(action: String): Int? {
        val words = action.lowercase().split(" ")
        return words.mapNotNull { it.toIntOrNull() }.firstOrNull()
    }
