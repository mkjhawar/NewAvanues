# Context Protocol: Avanues Platform Restructuring
...
|--------|-------|-----------|
| **Project Directory Name** | Avanues | Platform project container |
...
```
Tier 1: Avanues Platform (Pre-compiled runtime)
...
└── Avanues/apps/                  (App definitions, layouts)
```
...
### Current Location
- **Directory**: `/Volumes/M Drive/Coding/AvaCode` → **RENAME TO** `/Volumes/M Drive/Coding/Avanues`
...
**Configuration Files:**
1. `settings.gradle.kts` → `rootProject.name = "Avanues"`
...
**Modules to Migrate**:
1. keyboard/* (complete keyboard system) → Avanues/runtime/libraries/VoiceKeyboard/
2. voiceos-accessibility/ → Avanues/runtime/libraries/Accessibility/
3. voiceos-browser/ → Avanues/runtime/libraries/Browser/
4. voiceos-cloud/ → Avanues/runtime/libraries/CloudStorage/
5. voiceos-filemanager/ → Avanues/runtime/libraries/FileManager/
6. voiceos-task/ → Avanues/runtime/libraries/TaskManager/
7. voiceos-notepad/ → Avanues/runtime/libraries/Notepad/
8. remote-control/ → Avanues/runtime/libraries/RemoteControl/
9. bottom-command-bar/ → Avanues/runtime/libraries/CommandBar/
10. augmentalis_theme/ → Avanues/runtime/libraries/AvaUI/theme/ (merge)
11. color_picker/ → Avanues/runtime/libraries/ColorPicker/
12. app-preferences/ → Avanues/runtime/libraries/Preferences/
...
**Integration Needed**:
- Add composite build: `includeBuild("../Avanues")`
- Create microapp/ package for generating Avanues apps
...
2. **US2 - Avenue-Redux Library Migration**: Migrate 15+ modules to Avanues platform
...
**Benefit**: VoiceOS automatically builds and includes Avanues libraries without publishing to Maven. Changes in Avanues are immediately available in VoiceOS builds.
...
### Don't Forget
1. **Namespace is VoiceOS**, not Avanues (project name ≠ namespace)
...
- [ ] Verify directory renamed: `ls "/Volumes/M Drive/Coding/Avanues"`
