...
class VoiceRecognitionService : Service() {
    ...
    private suspend fun initializeAndStartRecognition(
        ...
        try {
            ...
            when (val eng = currentEngine) {
                is AndroidSTTEngine -> eng.startListening(mode)
                is VoskEngine -> eng.startListening()
                is VivokaEngine -> eng.startListening()
                // GoogleCloudEngine disabled - handled by fallback
                is WhisperEngine -> eng.startListening()
                else -> {
    ...
    private suspend fun stopCurrentRecognition() {
        try {
            when (val eng = currentEngine) {
                is AndroidSTTEngine -> eng.stopListening()
                is VoskEngine -> eng.stopListening()
                is VivokaEngine -> eng.stopListening()
                // GoogleCloudEngine disabled - handled by fallback
                is WhisperEngine -> eng.stopListening()
                else -> { /* No-op */ }
            }
