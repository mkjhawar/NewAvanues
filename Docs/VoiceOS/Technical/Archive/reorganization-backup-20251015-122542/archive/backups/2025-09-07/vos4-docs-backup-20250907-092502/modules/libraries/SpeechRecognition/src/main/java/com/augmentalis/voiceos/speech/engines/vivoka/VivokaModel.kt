...
class VivokaModel(
    ...
    suspend fun initializeModel(
        ...
        return try {
            ...
            modelErrorListener?.invoke("Model initialization failed: ${e.message}", SpeechError.MODEL_LOADING_ERROR)
            isModelReady = false
            false
        }
    }
    ...
    suspend fun setModelPath(modelPath: String): Boolean {
        return try {
            ...
            modelErrorListener?.invoke("Model path setting failed: ${e.message}", SpeechError.MODEL_LOADING_ERROR)
            false
        }
    }
    ...
    suspend fun compileModelWithCommands(commands: List<String>): Boolean {
        ...
        return compilationMutex.withLock {
            try {
                ...
                modelErrorListener?.invoke("Model compilation failed: ${e.message}", SpeechError.MODEL_LOADING_ERROR)
                false
            } finally {
                isCompiling = false
            }
    ...
    suspend fun switchToDictationModel(dictationModelPath: String): Boolean {
        return try {
            ...
            modelErrorListener?.invoke("Dictation model switch failed: ${e.message}", SpeechError.MODEL_LOADING_ERROR)
            false
        }
    }
    ...
    suspend fun switchToCommandModel(commandModelPath: String): Boolean {
        return try {
            ...
            modelErrorListener?.invoke("Command model switch failed: ${e.message}", SpeechError.MODEL_LOADING_ERROR)
            false
        }
    }
