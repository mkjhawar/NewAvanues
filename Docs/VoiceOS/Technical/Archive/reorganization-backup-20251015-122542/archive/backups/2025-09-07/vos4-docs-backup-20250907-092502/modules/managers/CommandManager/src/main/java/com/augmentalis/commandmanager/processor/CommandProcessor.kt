...
class CommandProcessor(
    ...
    suspend fun initialize() {
        ...
        android.util.Log.i(TAG, "Command processor initialized with ${actionRegistry.size} actions")
    }
    ...
    suspend fun shutdown() {
        ...
        android.util.Log.i(TAG, "Command processor shutdown")
    }
    ...
    suspend fun processCommand(
        ...
        try {
            ...
            android.util.Log.e(TAG, "Error processing command: $text", e)
            return CommandResult(
                success = false,
                command = Command(
                    id = "error",
    ...
    suspend fun executeCommand(command: Command): CommandResult {
        ...
        try {
            ...
            android.util.Log.d(TAG, "Executed command: ${command.id} -> ${result.success}")

            return result.copy(executionTime = System.currentTimeMillis() - startTime)

        } catch (e: Exception) {
            android.util.Log.e(TAG, "Error executing command: ${command.id}", e)
            return CommandResult(
                success = false,
                command = command,
    ...
    private fun registerActionHandlers() {
        ...
        // Cursor actions
        actionRegistry["cursor_click"] = Actions.ClickAction()::invoke
        actionRegistry["cursor_double_click"] = Actions.DoubleClickAction()::invoke
        actionRegistry["cursor_long_press"] = Actions.LongPressAction()::invoke
        actionRegistry["cursor_show"] = Actions.ShowCursorAction()::invoke
        actionRegistry["cursor_hide"] = Actions.HideCursorAction()::invoke
        actionRegistry["cursor_center"] = Actions.CenterCursorAction()::invoke
        actionRegistry["cursor_hand"] = Actions.HandCursorAction()::invoke
        actionRegistry["cursor_normal"] = Actions.NormalCursorAction()::invoke
        actionRegistry["cursor_move"] = Actions.MoveCursorAction()::invoke
