...
class TestSpeechActivity : Activity() {
    ...
    private fun createUI(): LinearLayout {
        return LinearLayout(this).apply {
            ...
            statusText = TextView(context).apply {
                text = "Accessibility: ${if (VoiceAccessibilityService.isServiceRunning()) "✓ Connected" else "✗ Not Connected"}"
                textSize = 16f
                setTextColor(if (VoiceAccessibilityService.isServiceRunning()) Color.GREEN else Color.RED)
                gravity = Gravity.CENTER
                setPadding(0, 0, 0, 24)
            }
            addView(statusText)
    ...
    private fun processCommand(command: String) {
        runOnUiThread {
            ...
            // Direct call to VoiceAccessibilityService - zero overhead
            val executed = VoiceAccessibilityService.executeCommand(command)
    ...
    override fun onResume() {
        super.onResume()
        statusText.text = "Accessibility: ${if (VoiceAccessibilityService.isServiceRunning()) "✓ Connected" else "✗ Not Connected"}"
        statusText.setTextColor(if (VoiceAccessibilityService.isServiceRunning()) Color.GREEN else Color.RED)
    }
