...
class GazeTracker(
    ...
    private var lastGazeTime: Long? = null
    ...
    private fun calculateGazeFromFace(face: com.google.mlkit.vision.face.Face) {
        ...
        if (leftEye != null && rightEye != null) {
            ...
            _currentGaze.value = GazeTarget(
                elementId = "gaze_target_${System.currentTimeMillis()}",
                position = Vector3D(calibratedX, calibratedY, -1.0f),
                confidence = gazeAccuracy,
                dwellTime = System.currentTimeMillis() - (lastGazeTime ?: System.currentTimeMillis())
            )
    ...
    private fun updateGazeStability(gaze: GazeTarget) {
        ...
        val ageMs = System.currentTimeMillis() - gaze.dwellTime
        val stability = when {
            ageMs < 100 -> 1.0f
            ageMs < 500 -> 0.8f
            ageMs < 1000 -> 0.6f
    ...
    private fun isGazeIntersecting(gaze: GazeTarget, element: UIElement): Boolean {
        ...
        return gaze.position.x >= (element.bounds.left - tolerance) &&
               gaze.position.x <= (element.bounds.right + tolerance) &&
               gaze.position.y >= (element.bounds.top - tolerance) &&
               gaze.position.y <= (element.bounds.bottom + tolerance)
    }
    ...
    fun calibrateGaze(actualTarget: UIElement, gazeTarget: GazeTarget) {
        ...
        val errorX = actualTarget.center.x - gazeTarget.position.x
        val errorY = actualTarget.center.y - gazeTarget.position.y
        ...
        val error = sqrt(errorX * errorX + errorY * errorY).toFloat()
