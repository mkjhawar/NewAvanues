 ...
 * Created: 2025-09-03
 * 
 * Handles pinch, zoom, drag, and complex path-based gestures.
 * Implements ActionHandler interface (approved VOS4 exception).
 * Migrated from Legacy Avenue GestureActions.kt with 100% functional equivalence.
 */
...
class GestureHandler(
    ...
    override fun getSupportedActions(): List<String> {
        return listOf(
            ...
            "drag <coordinates>", "swipe <direction>",
            "gesture <path>", "path gesture <points>"
        )
    }
    ...
    fun performClickAt(x: Float, y: Float): Boolean {
        return try {
            ...
            val tap = GestureDescription.StrokeDescription(path, 0, tapDuration)
            val gesture = GestureDescription.Builder()
                .addStroke(tap)
                .build()
