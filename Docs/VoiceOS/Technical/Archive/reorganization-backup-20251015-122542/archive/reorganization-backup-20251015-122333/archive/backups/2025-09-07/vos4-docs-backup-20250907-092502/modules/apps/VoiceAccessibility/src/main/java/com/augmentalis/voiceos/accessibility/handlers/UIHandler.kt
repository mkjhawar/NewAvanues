 ...
 * Created: 2025-01-27
 * 
 * Handles clicking, tapping, and interacting with UI elements.
...
class UIHandler(
    ...
    override fun execute(
        ...
        return when {
            ...
            normalizedAction.startsWith("select ") -> {
                val target = normalizedAction.removePrefix("select ").trim()
    ...
    private fun performSelect(target: String): Boolean {
        val node = findNodeByText(target) ?: findNodeByDescription(target)
    ...
    /**
     * Find clickable node at specific coordinates
     */
    private fun findNodeAtCoordinates(x: Int, y: Int): AccessibilityNodeInfo? {
        val rootNode = service.rootInActiveWindow ?: return null
        return findNodeAtCoordinatesRecursive(rootNode, x, y)
    }
