...
- If not available:
  ...
  - Option 2: "Use idea-solo protocol instead (no multi-agent consultation)"
...
**DEFAULT BEHAVIOR:**
Multi-agent consultation is **ENABLED BY DEFAULT** when using IDEA protocol.
...
For single-agent workflow, use the idea-solo protocol instead.

**CUSTOM AGENTS:**
The user can specify different agents by saying: "use IDEA with consultation from [agent1] and [agent2]"
...
## Overview
IDEA is a structured development protocol that emphasizes specification-driven development with iterative implementation and continuous review. It builds upon the DAPPER methodology with a focus on context-first development and multi-agent collaboration.
...
### Branch Naming
```
idea/####-<spec-name>/<phase-name>
```

Example:
```
idea/0001-user-authentication/database-schema
```
