...
EVERY app MUST have a separate, swappable theming module that allows switching between IdeaMagic UI and standard platform/Material Design UI at runtime or build time.
...
- ❌ UI framework locked in (can't switch from Material to IdeaMagic)
...
- IdeaMagic base theme definitions (if multiple apps use identical Avanues branding)
...
│   │   │   │       │       │
│   │   │   │       │       ├── ideamagic/              # IdeaMagic implementation (PROJECT-SPECIFIC BRANDING)
│   │   │   │       │       │   ├── IdeaMagicTheme.kt   # AVA blue / Avanues purple / etc.
│   │   │   │       │       │   ├── IdeaMagicColors.kt  # Custom accent colors per app
│   │   │   │       │       │   ├── IdeaMagicTypography.kt
│   │   │   │       │       │   └── IdeaMagicComponents.kt
│   │   │   │       │       │
...
// ava/app/src/main/kotlin/.../ui/theme/ideamagic/IdeaMagicColors.kt
class IdeaMagicColors(val isDark: Boolean) : AppColors {
...
// avanues/app/src/main/kotlin/.../ui/theme/ideamagic/IdeaMagicColors.kt
class IdeaMagicColors(val isDark: Boolean) : AppColors {
...
// avaconnect/app/src/main/kotlin/.../ui/theme/ideamagic/IdeaMagicColors.kt
class IdeaMagicColors(val isDark: Boolean) : AppColors {
 ...
 *
 * Implementations: IdeaMagic, Material Design, Custom
 */
interface AppColors {
    // Primary colors
    val primary: Color
...
### 2. IdeaMagic Theme Implementation

```kotlin
// app/src/main/kotlin/com/{app}/ui/theme/ideamagic/IdeaMagicColors.kt

/**
 * IdeaMagic color scheme implementation.
 *
 * Brand colors for Avanues ecosystem integration.
 */
class IdeaMagicColors(
    val isDark: Boolean
) : AppColors {
    ...
    // Avanues voice colors (signature IdeaMagic palette)
...
```kotlin
// app/src/main/kotlin/com/{app}/ui/theme/ideamagic/IdeaMagicTypography.kt

/**
 * IdeaMagic typography system.
 *
 * Uses custom "IdeaMagic Sans" font (fallback to Poppins/Roboto).
 */
class IdeaMagicTypography : AppTypography {
...
```kotlin
// app/src/main/kotlin/com/{app}/ui/theme/ideamagic/IdeaMagicTheme.kt

/**
 * IdeaMagic theme provider.
 *
 * Wraps app content with IdeaMagic theming.
 */
@Composable
fun IdeaMagicTheme(
    ...
    val colors = IdeaMagicColors(isDark = darkTheme)
    val typography = IdeaMagicTypography()
    val components = IdeaMagicComponents()
...
enum class ThemeType {
    IDEAMAGIC,      // IdeaMagic theme (Avanues ecosystem)
...
data class ThemeConfig(
    ...
    companion object {
         ...
         *
         * If yes, defaults to IdeaMagic theme.
...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            // Automatically selects theme (IdeaMagic if in Avanues, Material otherwise)
            AppTheme {
                AppNavigation()
            }
        }
...
@Composable
fun HomeScreen() {
    // Access theme colors (works with both IdeaMagic and Material)
...
**Build variants:**
- `ideamagicRelease` - IdeaMagic theme only (for Avanues ecosystem)
...
✅ **Seamless integration:** IdeaMagic theme matches Avanues launcher
...
- Implemented IdeaMagic and Material themes
