...
class VivokaRecognizer(
    ...
    suspend fun processRecognitionResult(
        ...
        onModeSwitch: suspend (RecognizerMode, String?) -> Unit
        ...
        return try {
            ...
            // Parse VSDK result
            val parsedResult = AsrResultParser.parseResult(result)
            var recognizerMode: RecognizerMode
            
            // Process all hypotheses
            for (hypothesis in parsedResult.hypotheses) {
                val command = cleanString(hypothesis.text ?: "")
                if (command.isEmpty()) continue
                
                val confidence = hypothesis.confidence
                
                Log.d(TAG, "Hypothesis: command='$command', confidence=$confidence, threshold=${config.confidenceThreshold}")
                
                // Check confidence threshold
                if (confidence >= config.confidenceThreshold * 100) {
                    
                    // Handle unmute command in sleep mode
                    ...
                    if (isDictationActive) {
                        ...
                        if (!isDictationEnding) {
                            ...
                            // Signal mode continuation for dictation
                            onModeSwitch(recognizerMode, null)
                            
                            return RecognitionProcessingResult.dictationContinue(speechResult)
                        } else {
                            // End dictation
                            recognizerMode = RecognizerMode.STOP_FREE_SPEECH
                            onModeSwitch(recognizerMode, null)
                            return RecognitionProcessingResult.dictationEnd()
                        }
                    } else {
                        // Handle command mode
                        if (command.equals(config.muteCommand, ignoreCase = true)) {
                            // Mute command
                            recognizerMode = RecognizerMode.COMMAND
                            onModeSwitch(recognizerMode, null)
                            return RecognitionProcessingResult.muteCommand()
                            
                        } else if (command.equals(config.startDictationCommand, ignoreCase = true)) {
                            // Start dictation
                            recognizerMode = RecognizerMode.FREE_SPEECH_START
                            onModeSwitch(recognizerMode, null)
                            return RecognitionProcessingResult.dictationStart()
                            ...
                            // Process with delay for stability
                            startCommandProcessing(speechResult)
                            
                            // Signal mode switch for command processing
                            onModeSwitch(recognizerMode, null)
                            
                            return RecognitionProcessingResult.regularCommand(speechResult)
                        }
                    }
                } else {
                    // Low confidence - send as partial result
            ...
            RecognitionProcessingResult.noResult()
    ...
    /**
     * Create RecognitionResult from recognition data
     */
    private fun createRecognitionResult(
        ...
        return RecognitionResult(
            text = text,
            confidence = confidence / 100f, // Convert percentage to float
            isFinal = isFinal,
            engine = "vivoka",
