...
The AVID hash is deterministic: `hash(className + resourceId + text + contentDescription)`.
...
**Hash inputs:** `className + resourceId + text + contentDescription`

**Critical property:** AVID does NOT include bounds/position. The same button produces
the same AVID on Pixel 7, Galaxy Fold, iPad-sized tablet, portrait or landscape.
