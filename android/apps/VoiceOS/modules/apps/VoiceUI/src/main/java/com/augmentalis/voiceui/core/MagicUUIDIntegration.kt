...
object MagicUUIDIntegration {
    ...
    fun findComponent(
        ...
        return when {
            ...
            type != null -> {
                val elements = uuidCreator.findByType("magic_component_$type")
                elements.firstOrNull()?.uuid?.let { componentMetadata[it] }
            }
            screenUUID != null -> {
    ...
    fun navigateToComponent(
        ...
        return uuidCreator.findInDirection(fromUUID, directionString)?.uuid
    }
    ...
    fun executeVoiceCommand(
        ...
        scope.launch {
            uuidCreator.executeAction(targetUUID, action, parameters)
        }
    }
