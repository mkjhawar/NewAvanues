...
class WhisperModelManager(private val context: Context) {
    ...
    private val pathResolver = SpeechModelPathResolver(context, "whisper", MODELS_DIR)
    ...
    suspend fun downloadModel(modelSize: WhisperModelSize) = withContext(Dispatchers.IO) {
        ...
        if (isModelDownloaded(modelSize)) {
            ...
            val path = getModelPath(modelSize) ?: run {
                _downloadState.value = ModelDownloadState.Error("Model path not found for: ${modelSize.modelName}")
                return@withContext
            }
            _downloadState.value = ModelDownloadState.Completed(path)
            return@withContext
        }
    ...
    private fun getModelFile(fileName: String): File {
        // Use path resolver to find models directory with multi-location fallback
        Log.d(TAG, "Searching for model directory in locations:\n${pathResolver.getSearchPathsForLogging()}")

        // Validation function to check if directory contains the model file
        val validationFunction: (File) -> Boolean = { dir ->
            val modelFile = File(dir, fileName)
            modelFile.exists() && modelFile.isFile
        }

        val modelsDir = pathResolver.resolveModelPath(validationFunction)
        val modelFile = File(modelsDir, fileName)

        // Log result
        if (modelFile.exists()) {
            Log.i(TAG, "Found model '$fileName' at: ${modelFile.absolutePath}")
        } else {
            Log.d(TAG, "Model '$fileName' not found, will download to: ${modelFile.absolutePath}")
        }
